...
public class StringUtil {
    ...
    public static int indexOfIgnore(String strTarget, String strDest, int nPos) {
        if (strTarget == null || strTarget.equals("")) return -1;
    ...
    public static String replace(String strTarget, String strOld, String strNew, boolean bIgnoreCase,
        ...
        if (bIgnoreCase == true) { // 대소문자 구분하지 않을 경우
            ...
            while ((nPos = strTarget.indexOf(strOld, nPosOld)) >= 0) {
                ...
                objDest.append(strNew);
                nPosOld = nPos + nLen;
                if (bOnlyFirst == true) break;
            }
        }

        if (nPosOld < strTargetLen) objDest.append(strTarget.substring(nPosOld, strTargetLen));
    ...
    public static String padValue(String strTarget, String strDest, int nSize, boolean bLeft) {
        if (strTarget == null) return strTarget;
    ...
    public static String formatMoney(String strTarget) {
        if (strTarget == null && strTarget.trim().length() == 0) return "0";
    ...
    public static String round(String strTarget, int nDotSize) {
        if (strTarget == null || strTarget.trim().length() == 0) return strTarget;
    ...
    public static String formatPost(String strTarget) {
        ...
        if (strValue.length() != 6) return strTarget;
     ...
     * @param strTarget
     *            대상 문자열
     * @return 문자열이 3자 이내일때 앞 2자 노출, 4자 이상일때 앞 3자 노출, 나머지 문자는 *로 치환하여 반환한다.
     */
    public static String maskingName(String strTarget) {
        ...
        if (strTarget == null || "".equals(strTarget)) {
        	...
        	if(strTarget.length() > 3){
        		retValue = strTarget.replaceAll("(?<=.{3}).", "*");
        	}else{
        		retValue = strTarget.replaceAll("(?<=.{2}).", "*");
        	}
        }
    ...
    public static String convertCharset(String s, String from, String to) throws IOException {
        byte[] fromStringBuffer = s.getBytes(Charset.forName(from));
        /*log.debug("{} - length : {}", from, fromStringBuffer.length);*/
        String sourceStr = new String(fromStringBuffer, from);
        /*log.debug("String from {} : {}", from, sourceStr);*/

        // String 을 타겟 캐릭터셋으로 인코딩.
        byte[] toStringBuffer = sourceStr.getBytes(to);

        /*log.debug("{} - length : {}", to, toStringBuffer.length);*/
        String targetStr = new String(toStringBuffer, to);
