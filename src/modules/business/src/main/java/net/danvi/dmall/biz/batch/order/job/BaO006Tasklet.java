 ...
 * 작성일     : 2016. 9. 30.
 * 작성자     : KNG
 * 설명       : 입금누락 Email 발송 처리 tasklet
 * </pre>
 */
public class BaO006Tasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
        ...
        for (int i = 0; i < OrdNoneDepositList.size(); i++) {
            batchOrderService.sendOrdAutoEmail("13", OrdNoneDepositList.get(i).get("SITE_NO").toString() , OrdNoneDepositList.get(i).get("ORD_NO").toString());
        }
