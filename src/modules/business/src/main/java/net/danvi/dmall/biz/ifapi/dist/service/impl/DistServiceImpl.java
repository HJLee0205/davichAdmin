package net.danvi.dmall.biz.ifapi.dist.service.impl;
...
import net.danvi.dmall.biz.ifapi.cmmn.mapp.dto.OrderMapRegReqDTO;
import net.danvi.dmall.biz.ifapi.cmmn.mapp.dto.ReturnMapRegReqDTO;
...
import net.danvi.dmall.biz.ifapi.dist.dto.OrderRegReqDTO.OrderDetailDTO;
import net.danvi.dmall.biz.ifapi.dist.dto.OrderReleaseReqDTO.OrderReleaseDtlDto;
import net.danvi.dmall.biz.ifapi.dist.dto.OrderReleaseResDTO.ReleaseFailDTO;
import net.danvi.dmall.biz.ifapi.dist.dto.RefundCmpltReqDTO.RefundItemDTO;
 ...
 * - 패키지명      : net.danvi.dmall.admin.ifapi.dist.service.impl
...
public class DistServiceImpl extends BaseService implements DistService {
...
//	@Transactional(transactionManager="transactionManager2", rollbackFor=Exception.class)
	...
	public synchronized void insertOrderInfo(OrderRegReqDTO param) throws Exception {
		...
		if(Constants.ORD_RUTE.STORE_RECV.equals(param.getDestType()) || Constants.ORD_RUTE.STORE_SELLER_RECV.equals(param.getDestType())) {
			// 매장 수령인 경우
			closingTime = proxyDao.selectOne("dist.selectClosingTimeForStoreDlvr");
		} else {
			// 집으로 배송인 경우
			closingTime = proxyDao.selectOne("dist.selectClosingTimeForHomeDlvr");
		}
		// 현재 시간
		...
		if(!param.isPartIfOrdReg()) {
			// 전표번호 취득
			String ordSlip = proxyDao.selectOne("dist.getNextOrdSlip", param);
		...
		// 발주 데이터 저장
		proxyDao.insert("dist.insertOrderInfo", param);
		...
		String curOrdDtlSeq = proxyDao.selectOne("dist.getCurOrdDtlSeq", param);
		// 발주 상세 데이터 저장
		for(OrderDetailDTO dtl : param.getOrdDtlList()) {
			...
			// 순번 설정
			dtl.setErpOrdDtlSeq(String.valueOf(ordDtlSeqMap.get(dtl.getItmCode()) + Integer.parseInt(curOrdDtlSeq)));
			...
			// 데이터 저장
			proxyDao.insert("dist.insertOrderDetailInfo", dtl);
		...
		for(OrderRegReqDTO.PayInfoDTO payDto : param.getPayList()) {
		...
		for(OrderDetailDTO dtlDto : param.getOrdDtlList()) {
			...
			if(payDto.getCouponUseList() != null && payDto.getCouponUseList().size() > 0) {
				...
				for(OrderPayInfoDTO.CouponUseDTO cpnUseDto : payDto.getCouponUseList()) {
					...
					proxyDao.insert("dist.insertOrderPaymentInfo", payDto);
				}
				// 할인 정보 제거
		...
		mappingService.insertOrderAndDtlMap(mapParam);
...
//	@Transactional(transactionManager="transactionManager2", rollbackFor= {Exception.class})
	public void cancelOrder(OrderCancelReqDTO param) throws Exception {
		proxyDao.update("dist.cancelOrder", param);

		OrderMapRegReqDTO mapParam = new OrderMapRegReqDTO();
		mapParam.setMallOrderNo(param.getOrderNo());

		mappingService.deleteOrderAndDtlMap(mapParam);
...
//		String resParam = sendUtil.send(mapParam, "deleteOrderMap");
	...
	public List<ReleaseFailDTO> insertReleaseInfo(OrderReleaseReqDTO param) throws Exception {
		...
		for(OrderReleaseDtlDto dtl : param.getReleaseList()) {
			...
			try {
				...
				if(mapDtlDto == null) {
					// 매핑되지 않은 주문상세번호입니다.
					throw new CustomIfException("ifapi.exception.order.orderdtlseq.notmapped");
				}
				...
				if (!chkModel.isSuccess()) {
					// 송장번호가 유효하지 않습니다.
					throw new CustomIfException("ifapi.exception.order.invoice.invalid");
				}

				Map<String, Object> paramMallOrder = new HashMap<>();
				paramMallOrder.put("mallOrderNo", dtl.getMallOrderNo());
				paramMallOrder.put("mallOrderDtlSeq", dtl.getMallOrdDtlSeq());
				// 쇼핑몰 배송 정보 조회
				Map<String, Object> mallDlvrInfo = proxyDao.selectOne("dist.selectMallOrderDlvrInfo", paramMallOrder);
				...
				if(!isSuccess) {
					// 출고정보 등록에 실패했습니다.
					throw new CustomIfException("ifapi.exception.order.release.failed");
				}
	...
	public void completeStoreDelivery(StoreDlvrCmpltReqDTO param) throws Exception {
		...
		for(String mallOrdDtlSeq : param.getMallOrdDtlSeq()) {
			...
			Map<String, String> paramMallOrdDtl = new HashMap<>();
			paramMallOrdDtl.put("ordNo", param.getMallOrderNo());
			paramMallOrdDtl.put("ordDtlSeq", mallOrdDtlSeq);
			// 해당 데이터의 현재 상태를 확인 - 구매확정 상태이면 수정 안함.
			String currStatusCd = proxyDao.selectOne("dist.selectOrderDtlCurrentStatus", paramMallOrdDtl);
...
//			vo.setDlvrMsg(dlvrMsg);
	...
	public void insertMallReturnReq(OrderRegReqDTO param) throws Exception {
		...
		mappingService.insertReturnMap(mapParam);
		BaseResDTO resDto = new BaseResDTO();
		resDto.setResult(Constants.RESULT.SUCCESS);
		resDto.setMessage("");
		String resParam = JSONObject.fromObject(resDto).toString();
...
//	@Transactional(transactionManager="transactionManager2")
	public void updateErpReturnComfirm(MallReturnConfirmReqDTO param) throws Exception {
		// 발주 상세 상태 갱신
		for(MallReturnConfirmReqDTO.ClaimInfoDTO dto : param.getClaimList()) {
			// 파라미터에 상태코드 설정 - 반품확정
			dto.setStatus(Constants.RETURN_STATUS.RETURN_CONFIRM);

			proxyDao.update("dist.updateErpOrderDtlStatus", dto);
		}
		// 반품 확정 되지 않은 데이터 개수
		int notConfirmCnt = proxyDao.selectOne("dist.countErpOrderDtlOtherStatus", param.getClaimList().get(0));
		
		if(notConfirmCnt == 0) {
			// 모두 확정이면 마스터도 확정으로 갱신
			proxyDao.update("dist.updateErpOrderStatus", param.getClaimList().get(0));
		}
	}
...
//	@Transactional(transactionManager="transactionManager1", rollbackFor=CustomException.class)
	public void updateMallReturnConfirm(ErpReturnConfirmReqDTO param) throws Exception {
		...
		proxyDao.update("dist.updateMallReturnStatus", param);
...
//	@Transactional(transactionManager="transactionManager2", rollbackFor=CustomException.class)
	public void updateErpPurchaseConfirm(PurchaseConfirmReqDTO param) throws Exception {
		...
		int resultCnt = proxyDao.update("dist.updateErpPurchaseConfirm", param);
		
		if(resultCnt != param.getOrdDtlList().size()) {
			// 구매확정 정보가 일치하지 않습니다.
			throw new CustomIfException("ifapi.exception.purchseconfirm.invaliddata");
		}
	}
	...
	public void completeRefund(RefundCmpltReqDTO param) throws Exception {
		...
		Map<String, String> paramCharge = new HashMap<>();
		paramCharge.put("ordDate", param.getOrdDate());
		paramCharge.put("strCode", param.getStrCode());
		paramCharge.put("ordSlip", param.getOrdSlip());
		proxyDao.update("dist.updateOrderChargeYnToY", paramCharge);
	}
	...
	private List<OrderPayInfoDTO> insertRefundPayInfo(String ordDate, String strCode, String ordSlip, String ordSeq, String ordAddNo, String payDate) throws Exception {
		...
		ReturnInfoDTO rtnDtl = proxyDao.selectOne("dist.selectReturnOrderDtlInfo", searchParam);
		
		// 반품 발주의 원 발주 수량조회(기 환불완료된거 마이너스 처리 해서 조회)
		int ordQty = proxyDao.selectOne("dist.selectOrgOrderQtyExceptRefund", rtnDtl);
		
		// 반품 발주의 원 발주 결제 정보 조회 (기 환불완료된거 마이너스 처리 해서 조회)
		List<OrderPayInfoDTO> orgPayList = proxyDao.selectList("dist.selectOrgPaymentInfoListExceptRefund",rtnDtl);
		...
		if(rtnDtl.getSupQty() == ordQty) {
			...
			for(OrderPayInfoDTO payInfoDto : orgPayList) {
				...
				proxyDao.insert("dist.insertOrderPaymentInfo", payInfoDto);
	...
	public void completeChange(OrderRegReqDTO param) throws Exception {
		...
		for(OrderDetailDTO dtl : param.getOrdDtlList()) {
			...
			// 데이터 저장
			proxyDao.insert("dist.insertOrderDetailInfo", param);
			...
			for(OrderPayInfoDTO payInfoDto : payInfoList) {
				...
				proxyDao.insert("dist.insertOrderPaymentInfo", param);
			}
		}
		
		// 반품 발주 정보에 charge_yn을 'Y'로 설정
		Map<String, String> paramCharge = new HashMap<>();
		paramCharge.put("ordDate", param.getOrgOrdDate());
		paramCharge.put("strCode", param.getOrgStrCode());
		paramCharge.put("ordSlip", param.getOrgOrdSlip());
		proxyDao.update("dist.updateOrderChargeYnToY", paramCharge);
