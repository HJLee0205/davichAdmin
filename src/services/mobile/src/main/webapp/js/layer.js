...
Dmall.LayerUtil = {
    ...
    create : function($layer) {
        ...
        if(dimmed) {
            ...
            $layer.css('z-index', 1200).find('.pop_wrap').css('z-index', 1200);
        } else {
            $layer.prepend('<div class="dimmed2"></div>');
        }
    ...
    confirm : function(msg, yesFunc, noFunc, title, desc, left_btn, right_btn) {
        ...
        $('#btn_id_confirm_no, .alert_body button.btn_alert_close').on('click', function() {
            funciton2();
            Dmall.LayerUtil.close('div_id_confirm')
        });
    }
...
Dmall.LayerPopupUtil = {
    ...
    open : function($popup) {
        if(!$popup.hasClass('layer_popup')) {
            $popup.addClass('layer_popup').children().wrapAll('<div class="pop_wrap"></div>');
        }
        ...
        $popup.fadeIn();
        /*$popup.css({top: top, left: left, width: 'calc(97%)',position:'absolute'});*/
        $popup.css({top: top,position:'absolute'});
        if(dimmed) {
            $popup.prepend('<div class="dimmed2"></div>');
            $popup.css('z-index', 120).find('.pop_wrap').css('z-index', 120);
        } else {
            $popup.prepend('<div class="dimmed2"></div>');
        }
//        $('body').css('overflow-x','hidden').on('touchmove', function(e){e.preventDefault()});
...
//        $body.css('overflow-x','scroll').off('touchmove')
    },
    /**
     * <pre>
     * 함수명 : zipcode
    ...
    zipcode :function(callback) {
        var template =
                ...
                '</div>',*/
        	   '<!--- popup_주소찾기 --->' +
		        '<div id="popup_address" class="popup">' +
		        '	<div class="popup_header">' +
		        '		<h1 class="popup_tit">우편번호찾기</h1>' +
		        '		<button type="button" class="btn_close_popup closepopup"><span class="icon_popup_close"></span></button>' +
		        '	</div>' +
/*		        '	<div class="post_search">' +
		        '		<input type="text"><button>검색</button>' +
		        '		<ul class="post_search_info">' +
		        '			<li>※ 도로명주소로 검색 [시&middot;도/시&middot;군&middot;구 + 도로명주소]</li>' +
		        '			<li>※ 지번으로 검색 [시&middot;도/시&middot;군&middot;구/읍&middot;면&middot;동 + 지번]</li>' +
		        '			<li>※ 건물명으로 검색 [시&middot;도/시&middot;군&middot;구 + 건물명]</li>' +
		        '		</ul>' +
		        '	</div>' +*/
		        '<div class="pop_con post" style="height:460px">' +
		        '	<div class="post_scroll" id="layer_id_postList">' +
/*		        '		<ul class="post_result_list">' +
		        '			<li><span class="post_result_no">100-808</span> <p class="post_result_add">서울 중구 충무로2가 충무로2가 충무로2가 3-8</p></li>' +
		        '			<li><span class="post_result_no">100-808</span> 서울 중구 충무로2가 3-8</li>' +
		        '			<li><span class="post_result_no">100-808</span> 서울 중구 충무로2가 3-8</li>' +
		        '			<li><span class="post_result_no">100-808</span> 서울 중구 충무로2가 3-8</li>' +
		        '			<li><span class="post_result_no">100-808</span> 서울 중구 충무로2가 3-8</li>' +
		        '			<li><span class="post_result_no">100-808</span> 서울 중구 충무로2가 3-8</li>' +
		        ...
		        '	</div>' +
		        '	</div>' +
/*		        '	<div class="popup_btn_area" style="margin-top:15px">' +
		        '		<button type="button" class="btn_popup_ok">확인</button>' +
		        '		<button type="button" class="btn_popup_ok closepopup">닫기</button>' +
		        '	</div>' +*/
		        '</div> ' +
		        '<!---// popup_주소찾기 --->',
            elementLayer,
        ...
        elementLayer.style.width = width + '%';
        elementLayer.style.height = height + '%';
