...
jQuery(document).ready(function() {
    ...
    $('#a_id_logout').on('click', function(e) {
        ...
        Dmall.FormUtil.submit(MOBILE_CONTEXT_PATH+'/front/login/logout', {});
    });
  ...
  // 로컬에서 개발시 개발에 이미지가 없으면 테스트서버의 이미지를 보여주기위한 임시 처리
    /*jQuery('img').on('error', function(e) {
        ...
        if(hostname.indexOf('test.com') > 0) {
            hostname = hostname.replaceAll('id1.test.com', "www.davichmarket.com")
...
Dmall.waiting = {
     ...
     * <pre>
     * 함수명 : start
     * 설  명 : 화면 전체에 결제처리 중 처리
     * 사용법 : waiting.payStart()
     * 작성일 : 2016. 4. 28.
     * 작성자 : dong
     * 수정내역(수정일 수정자 - 수정내용)
     * -------------------------------------
...
Dmall.AjaxUtil = {
    ...
    getPayCert : function(url, param, callback) {
        Dmall.waiting.start();
        $.ajax({
            type : 'post',
            url : url,
...
Dmall.FileUpload = {
     ...
     * 수정내역(수정일 수정자 - 수정내용)
     * -------------------------------------
     * 2016. 5. 16. dong - 최초 생성
     * </pre>
     * @returns
     */
    file : function() {
        return Dmall.FileUpload.fileForm("file");
    },
    /**
     * <pre>
     * 함수명 : excel
     ...
     */
    excel : function() {
        return Dmall.FileUpload.fileForm("xls");
    },
    /**
     * <pre>
     * 함수명 : image
     ...
     */
    image : function(){
        return Dmall.FileUpload.fileForm("image");
    },
    /**
     * <pre>
     * 함수명 : fileForm
    ...
    upload : function(formId) {
        var dfd = jQuery.Deferred(),
            $form = $('#' + formId),
            url = $form.attr("action") || '/admin/common/file-upload';
        ...
        for(var i = 0, len = $files.length; i < len; i++) {
            ...
            for(var j = 0, l = files.length; j < l; j++) {
                ...
                if(file.size > Constant.file.maxSize) {
                    Dmall.LayerUtil.alert('파일 ' + file.name + '의 파일사이즈가 5MB를 초과합니다.');
                    return false;
                }
            }
        }
...
Dmall.FileDownload = {
    download : function(filePath, fileName) {
        var url = MOBILE_CONTEXT_PATH+'/front/common/common-download',
        key,
        param = {};
...
Dmall.DaumEditor = {
     ...
     */
    init : function() {
        var url = MOBILE_CONTEXT_PATH+'/front/daumeditor/editor.html';
    ...
    create2 : function(id, conf) {
        ...
        var config = {
            ...
            txIconPath: "/m/front/daumeditor/images/icon/editor/", /*에디터에 사용되는 이미지 디렉터리, 필요에 따라 수정한다. */
            txDecoPath: "/m/front/daumeditor/images/deco/contents/", /*본문에 사용되는 이미지 디렉터리, 서비스에서 사용할 때는 완성된 컨텐츠로 배포되기 위해 절대경로로 수정한다. */
            canvas: {
                exitEditor:{
                    /*
