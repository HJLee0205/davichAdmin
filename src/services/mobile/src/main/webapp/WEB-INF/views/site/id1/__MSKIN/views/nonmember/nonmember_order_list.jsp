...
<t:insertDefinition name="davichLayout">
    ...
    <div id="middle_area">
            ...
            <c:when test="${order_list.resultList ne null && fn:length(order_list.resultList) gt 0}">
                ...
                <c:forEach var="resultList" items="${order_list.resultList}" varStatus="status">
                    <c:forEach var="goodsList" items="${resultList.orderGoodsVO}" varStatus="status">
                        <!-- 구매내역 1set -->
                        <div class="my_order">
                            <a href="#" onclick="move_nonmember_order_detail('${resultList.orderInfoVO.ordNo}');">
        ...
        <c:forEach var="goodsList" items="${resultList.orderGoodsVO}" varStatus="status">
			<div class="my_order">
				<a href="javascript:;" onclick="move_nonmember_order_detail('${resultList.orderInfoVO.ordNo}');">
					<c:if test="${ordNo!= resultList.orderInfoVO.ordNo}">
					<div class="my_order_no">
						<c:if test="${resultList.orderInfoVO.orgOrdNo ne null}">
						<button type="button" class="btn_review_go" onclick="move_nonmember_order_detail('${resultList.orderInfoVO.ordNo}');">교환</button>
						</c:if>
						주문번호 :${resultList.orderInfoVO.ordNo}
					</div>
					</c:if>
				...
				<ul class="my_order_info_top">
					<li class="my_order_product_pic"><a href="${_MOBILE_PATH}/front/goods/goods-detail?goodsNo=${goodsList.goodsNo}"><img src="${goodsList.imgPath}" alt=""></a></li>
				...
				주문번호 <a href="#1" onclick="move_nonmember_order_detail('${resultList.orderInfoVO.ordNo}');">${resultList.orderInfoVO.ordNo}</a> -->
