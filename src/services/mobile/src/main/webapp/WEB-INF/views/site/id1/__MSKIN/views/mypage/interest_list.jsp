...
<t:insertDefinition name="davichLayout">
	...
	<div id="middle_area">	
		...
		<ul class="product_list_typeA interest">
		<c:choose>
	       <c:when test="${resultListModel.resultList ne null}">
	           ...
	           <input type="hidden" name="itemNoArr" value="${resultModel.itemNo}"/>
				<li data-goods-no="${resultModel.goodsNo}" data-item-no="${resultModel.itemNo}" data-stock-qtt="${resultModel.stockQtt}"
					...
					<div class="product_info">
						<button type="button" class="direct-delete-btn">삭제</button>
						<c:choose>
                             <c:when test="${resultModel.stockQtt eq 0}">
                                 <span class="label_soldout" style="margin-right:3px">품절</span>
                             </c:when>
                             <c:otherwise>
                                 <button type="button" class="direct-basket-btn" style="margin-right:3px">장바구니</button>
                             </c:otherwise>
                         </c:choose>
					</div>
            ...
            <c:otherwise>
                 <li class="no_product"">등록된 관심상품이 없습니다.</li>
            </c:otherwise>
        </c:choose>
		</ul>
		 <!---- 페이징 ---->
