...
<t:insertDefinition name="popupLayout">
    <t:putAttribute name="title">이용약관 동의</t:putAttribute>
    
    <t:putAttribute name="script">
    <script>
    $(document).ready(function(){
    	<c:if test="${param.keyData ne null and param.keyData ne ''}">
		var url = '${_MOBILE_PATH}/front/terms-apply-signinfo';
		...
		$('.btn_go_next').on('click',function(){
			...
			var param = $('#form_id_join').serializeArray();
			var url = '${_MOBILE_PATH}/front/member-terms-apply';
			Dmall.AjaxUtil.getJSON(url, param, function(result) {
				 if(result.success)
    ...
    <t:putAttribute name="content">
		...
		<h1>고객님 환영합니다!</h1>
		<div class="text">
			다비치안경체인 서비스 내 이용자 식별과 회원관리 서비스 제공을 위해 <br>
			아래 약관에서 필요한 사항을 규정합니다. 해당 정보는 언제든지 서비스 탈퇴 시  <br>
			지체없이 파기됩니다. <br>
				<%--
...
<%--		</div>--%>
			<input type="hidden" class="agree_check" name="rule22Agree" id="agree_check02" value="Y">
			<input type="hidden" class="agree_check" name="rule21Agree" id="agree_check03" value="Y">
			<input type="hidden" class="agree_check" name="rule09Agree" id="agree_check04" value="Y">
		<div class="alrim_talk_check">
			<c:if test="${applyCnt<1}">
			<input type="checkbox" class="agree_check" name="rule10Agree" id="agree_check05" value="Y">
			</c:if>
			<label for="agree_check05"><span></span>멤버십 및 다비치몰 이용약관<c:if test="${applyCnt<1}">(필수)</c:if></label>
		...
		<div class="alrim_talk_check" style="border-bottom:none">
			...
			<label for="sms_get">
				<span></span>혜택 알림 등 이메일 및 문자 정보 수신 동의<c:if test="${applyCnt<1}">(선택)</c:if>
				<p>고객님께 생일 쿠폰, 재검안 안내, 안경 완성 알림, A/S 관련 문자를 보내드리기 위해 동의 도와드리겠습니다.</p>
			</label>
		</div>
		</c:if>
		...
		<c:if test="${applyCnt<1}">
		<button type="button" class="btn_talk_agree btn_go_next">도의하고 계속하기</button>
		</c:if>
		<c:if test="${applyCnt >= 1}">
		<button type="button" class="btn_talk_agree" onclick="window.close(); location.href = 'kakaotalk://inappbrowser/close'; return false;">창 닫기</button>
	...
	<%--	<div>
		...
		<div class="btn_area">
            <button type="button" class="btn_go_next">동의</button>
        </div>--%>
