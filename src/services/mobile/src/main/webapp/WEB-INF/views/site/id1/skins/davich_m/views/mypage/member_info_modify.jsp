...
<t:insertDefinition name="davichLayout">
	<t:putAttribute name="title"><c:choose><c:when test="${so.integrationMemberGbCd eq '02'}">정회원 전환</c:when><c:otherwise>회원정보수정</c:otherwise></c:choose></t:putAttribute>
    ...
    $(document).ready(function(){
        ...
        if(temp_tel[0]!="undefinded") {
            $('#tel01').val(temp_tel[0]);
        }
        if(temp_tel[1]!="undefinded") {
            $('#tel02').val(temp_tel[1]);
        }
        if(temp_tel[2]!="undefinded") {
    ...
    function successIdentity(){
        var url = '${_MOBILE_PATH}/front/member/identity-success';
       	...
       	Dmall.LayerUtil.confirm(confirmTxt+" 하시겠습니까?",
       		function() {
            	...
            	$('#form_id_update').ajaxSubmit({
            		url : '${_MOBILE_PATH}/front/member/member-update',
            		dataType : 'json',
    ...
    <div id="middle_area">
        ...
        <div class="product_head">
			...
			<c:choose><c:when test="${so.integrationMemberGbCd eq '02'}">정회원 전환</c:when><c:otherwise>회원정보수정</c:otherwise></c:choose>
		</div>
        ...
        <form:hidden path="loginId" id="loginId" value="${resultModel.data.loginId}"/>
		...
		<div class="">
	        ...
	        <table class="tInsert join">
				<caption>회원정보 수정 입력폼입니다.</caption>
				<colgroup>
					<col style="width:30%">
					<col style="width:">
				</colgroup>
				<tbody>
								...
								<c:when test="${resultModel.data.joinPathCd eq 'SHOP' }">
									<c:choose>
										<c:when test="${resultModel.data.idChgYn eq 'N' }">
											<input type="text" id="chgId" name="chgId" value="${resultModel.data.loginId}" readonly="readonly" style="width: calc(100% - 95px);">
											<input type="hidden" id="idChgYn" name="idChgYn" value="${resultModel.data.idChgYn}">
											<button type="button" class="btn_password btn_id_check">아이디변경</button>
										</c:when>
										<c:otherwise>
					...
					<c:if test="${resultModel.data.joinPathCd eq 'SHOP' }">
			            <tr>
			                <th>비밀번호</th>
			                <td>
			                    <input type="password" id="pw" name="pw" onkeyup="passwordInputCheck();" style="    width: calc(100% - 95px);" maxlength="16">
					...
					<tr>
						...
						<td>
							<input type="text" id="email01"> @ <input type="text" id="email02"><br>
							<!-- <div class="select_box28" style="display:inline-block"> -->
								<label for="email03" style="display:none"></label>
								<select id="email03" title="select option">
					...
					<tr>
		                <th class="vaT">주소</th>
		                <td>
		                    <input type="text" id="newPostNo" name="newPostNo" value="${resultModel.data.newPostNo}" style="width:70px;margin-right:3px" readonly="readonly">
		                    <button type="button" class="btn_post btn_doubled_check">주소검색</button>
		                    <input type="hidden" id="strtnbAddr" name="strtnbAddr" value="${resultModel.data.strtnbAddr}" class="form_address"readonly="readonly">
		                    <input type="text" id="roadAddr" name="roadAddr" value="${resultModel.data.roadAddr}" class="form_address" readonly="readonly">
		                    <br>
		                    <input type="text" id="dtlAddr" name="dtlAddr" value="${resultModel.data.dtlAddr}" class="form_address" placeholder="상세주소">
		                </td>
		            </tr>
		            <c:if test="${resultModel.data.memberTypeCd eq '02'}">
					<tr>
		            ...
		            <tr>
		                ...
		                <td>
		                    <input type="checkbox" id="email_get" name="email_get" <c:if test="${resultModel.data.emailRecvYn eq 'Y'}">checked</c:if> class="agree_check">
		                    <label for="email_get"><span></span>이메일</label>
		                    <input type="checkbox" id="sms_get" name="sms_get" <c:if test="${resultModel.data.smsRecvYn eq 'Y'}">checked</c:if> class="agree_check">
        ...
        <div class="btn_area">
			<button type="button" class="btn_popup_ok" id="btn_change_member"><c:choose><c:when test="${so.integrationMemberGbCd eq '02'}">정회원 전환</c:when><c:otherwise>회원정보수정</c:otherwise></c:choose></button>
		</div>
	</div>
	<!--- popup 새 비밀번호 입력 --->
        ...
        <div class="popup_content">
            ...
            <table class="tMember_Insert">
                ...
                <tbody>
                    <tr>
                        <th>현재 비밀번호</th>
                        <td><input type="password" name="nowPw" id="nowPw" maxlength="16"></td>
                    </tr>
                    <tr>
                        <th>새 비밀번호</th>
                        <td><input type="password" name="newPw" id="newPw" maxlength="16"></td>
                    </tr>
                    <tr>
                        <th>새 비밀번호 확인</th>
                        <td><input type="password" name="newPw_check" id="newPw_check" maxlength="16"></td>
                    </tr>
                    <!--
                    <tr>
