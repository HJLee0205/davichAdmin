...
<t:insertDefinition name="davichLayout">
	...
	$(".btn_recom_again_new").click(function(){		
		document.location.href="${_MOBILE_PATH}/front/vision2/vision-check";
	});
	...
	$(".btn_recomm_save_new").click(function(){		
        ...
        if(!loginYn) {
            Dmall.LayerUtil.confirm("로그인이 필요한 서비스입니다.<br>지금 로그인 하시겠습니까?",
                ...
                function() {
                    /*var returnUrl = "${_MOBILE_PATH}/front/vision2/vision-check";
        ...
        Dmall.LayerUtil.confirm("결과를 저장하시겠습니까?", function(){
        	...
        	$.ajax({
    	 		type : "POST",
    	 		url : "${_MOBILE_PATH}/front/vision2/vision-check-insert",
    	 		data : {
	...
	function fn_incon1_img(imgPath){
		...
		$('.layer_lens_datil').show().prepend('<button type="button" class="btn_close_lendsdetail">창닫기</button>');		
		$(body).css('overflow-y', 'hidden');
	}	 

	$(".layer_lens_datil").click(function() {
	...
	<div id="middle_area">
		...
		<form:form id="form_vision_check" name="form_vision_check" method="post">
			...
			<input type="hidden" name="returnUrl" value="${_MOBILE_PATH}/front/vision2/vision-check-gr"/>
        </form:form>
		<div class="cont_body">
			...
			<c:forEach var="result" items="${visionGunList}" varStatus="status">
				<li>
					<p class="img">
						...
						<img src="${_IMAGE_DOMAIN}/image/image-view?type=VISION&path=${result.filePath}&id1=${result.fileNm}" alt="${result.gunNm}" onerror="this.src='../img/promotion/promotion_ing01.jpg'" style="height:auto;padding-top:13px">
						</c:if>
					</p>
					<div class="info">
						<p class="tit">	
