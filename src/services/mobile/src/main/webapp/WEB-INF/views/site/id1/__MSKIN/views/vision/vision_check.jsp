...
<t:insertDefinition name="davichLayout">
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$(".btn_view_recomm").click(function(){                	
				...
				Dmall.FormUtil.submit('${_MOBILE_PATH}/front/vision/recommend-lens', param);
		...
		function fn_select_age(type, idx) {
			...
			if(type == 'C'){
				var ageCd = '1'+idx+'';
				var url = "${_MOBILE_PATH}/front/vision/select-contact-ajax";
				...
				idx++;
				var ageCd = '0'+idx+'';
				var url = "${_MOBILE_PATH}/front/vision/select-poMatr-ajax";
                var param = {ageCd:ageCd};
                Dmall.AjaxUtil.getJSON(url, param, function(data) {
                    ...
                    for(var i=0;i<data.length;i++){
                    	...
                    	html += '<label for="poMatr'+i+'"><span></span>'+data[i].poMatrNm+'</label><br>';
                    }
		...
		function fn_select_poMatr(){
			...
			var url = "${_MOBILE_PATH}/front/vision/select-lifeStyle-ajax";
	...
	<t:putAttribute name="content">
		<!--- 02.LAYOUT: 카테고리 메인 --->
	    <div id="middle_area">
			<form:form id="form_vision_check" name="form_vision_check" method="post" style="height:0">
			...
			<div class="cont_body">
				...
				<div class="lens_type_area">
					...
					<div id="g_type_area" style="display:<c:if test="${lensType eq 'C' }">none</c:if>;">
						...
						<ul class="lens_choice glass_life" style="display:none;"></ul>
						<!--// 안경렌즈 라이프스타일 -->
					</div>
					<!--// 안경렌즈 영역 -->
					...
					<div id="c_type_area" style="display:<c:if test="${lensType ne 'C' }">none</c:if>;">
						...
						<ul class="lens_choice contact_life" style="display:none;"></ul>
					</div>
					<!--// 콘텐츠렌즈 영역 -->
