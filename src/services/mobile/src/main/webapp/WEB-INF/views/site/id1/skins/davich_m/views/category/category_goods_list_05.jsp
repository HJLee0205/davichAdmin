...
$(document).ready(function(){
    ...
    //베스트 브랜드
    	...
    	//1번째 카테고리
    	var ctgNo1 = '${param.ctgNo}';

    	if(ctgNo1=='' || ctgNo1=='747') {
		  ctgNo1 = $('.maga_category').find('a:eq(1)').attr("id");
		}
    	magazineList(ctgNo1);
...
var magazineList = function(ctgNo){
			...
			var url = '${_MOBILE_PATH}/front/search/category-ajax',dfd = jQuery.Deferred();
			var allflag = "";
			if(arguments[1]){
			    allflag =arguments[1];
			}
...
<t:putAttribute name="content">
	...
	<div class="magazine_outline_list"> <!-- 매거진 목록 div 추가 -->
		...
		<div class="cont_body">
			<div class="maga_category">
					<a href="javascript:magazineList('747','all');" id="747">전체</a>
			<%--	<c:forEach var="ctgList" items="${lnb_info.get('747')}" varStatus="status"> <!-- 1Depth -->
                <c:if test="${ctgList.ctgNo eq so.ctgNo || ctgList.upCtgNo eq '747'}">--%>
                    <c:forEach var="ctgList_1" items="${lnb_info.get('747')}" varStatus="status"> <!-- 2Depth -->
                    	<a href="javascript:magazineList('${ctgList_1.ctgNo}');" id="${ctgList_1.ctgNo}" style="padding;none;">${ctgList_1.ctgNm}</a>
                     </c:forEach>
              <%--  </c:if>
            	</c:forEach>--%>
		    ...
		    <%--	<c:forEach var="ctgList" items="${lnb_info.get('747')}" varStatus="status">
			${ctgList.ctgNm}
			</c:forEach>--%>
			...
			<!-- 목록 영역 -->
			<div class="magazine_list_area" data-masonry="{ &quot;columnWidth&quot;: &quot;.maga_box&quot;, &quot;itemSelector&quot;: &quot;.maga_box&quot;, &quot;gutter&quot;: 15 }" style="position: relative; height: 1594.17px;">
    ...
    <%--<div class="magazine_outline_list"> <!-- 매거진 목록 div 추가 -->
		<div class="category_middle">
			...
			<div class="maga_category">
				<a href="javascript:magazineList('${param.ctgNo}','all');" id="${param.ctgNo}">전체</a>
                ...
                <c:if test="${ctgList.ctgNo eq so.ctgNo || ctgList.upCtgNo eq so.ctgNo}">
                    <c:forEach var="ctgList_1" items="${lnb_info.get(ctgList.ctgNo)}" varStatus="status"> <!-- 2Depth -->
                    	<a href="javascript:magazineList('${ctgList_1.ctgNo}');" id="${ctgList_1.ctgNo}">${ctgList_1.ctgNm}</a>
                     </c:forEach>
                </c:if>
            </c:forEach>
			</div>
			...
			<div class="magazine_list_area" data-masonry="{ &quot;columnWidth&quot;: 375, &quot;itemSelector&quot;: &quot;.maga_box&quot;, &quot;gutter&quot;: 28 }" style="position: relative; height: 545px;">
				...
				<div class="maga_box" style="position: absolute; left: 0px; top: 20px;">
					...
					<div class="text_area">
						<p class="tit"><a href="javascript:goods_detail('${goodsList.goodsNo}');">${goodsList.goodsNm}</a></p>
						<p class="text"><a href="javascript:goods_detail('${goodsList.goodsNo}');">${goodsList.prWords}</a></p>
						<p class="tag">
							<c:set var="seoSearchWord" value="${fn:split(goodsList.seoSearchWord,',')}"/>
							<c:if test="${fn:length(seoSearchWord) gt 1}">
							<c:forEach var="hashTag" items="${seoSearchWord}" varStatus="g">
