...
<t:insertDefinition name="davichLayout">
                ...
                $(".btn_result_save").click(function(){
                    ...
                    if(!loginYn) {
                        Dmall.LayerUtil.confirm("로그인이 필요한 서비스입니다.<br>지금 로그인 하시겠습니까?",
                            ...
                            function() {
                                //var returnUrl = window.location.pathname+window.location.search;
                                var returnUrl = "${_MOBILE_PATH}/front/vision/vision-check2";                                
                                location.href= "${_MOBILE_PATH}/front/login/member-login?returnUrl="+returnUrl;
                            }
                        );
                        return;
	...
	<t:putAttribute name="content">
	    ...
	    <div class="middle_area">
			...
			<div class="cont_body">
				<div class="lens_recomm result">
						...
						<c:if test="${visionStepVO.visionGb == 'G' && step2Nm != 'skip'}">
							...
							<c:forEach  var="result" items="${step2NmList}" varStatus="status">
								<c:choose>
									...
									<c:otherwise>
										[<span onClick="fn_step2_img('${result.cd}')" style="cursor:pointer">${result.cdNm}</span>]
									</c:otherwise>
								</c:choose>							
							</c:forEach>					
					...
					<button type="button" class="btn_lens_show" onclick="document.location.href='${_MOBILE_PATH}/front/vision/vision-check2?visionGb=${visionStepVO.visionGb}'">렌즈추천 다시받기<i></i></button>
				</div>
	    ...
	    <!---// 02.LAYOUT: 카테고리 메인 --->
		<!-- popup_detail -->
		<div class="popup layer_lens_datil" style="display: none;">
		</div>
		<!--// popup_detail -->
