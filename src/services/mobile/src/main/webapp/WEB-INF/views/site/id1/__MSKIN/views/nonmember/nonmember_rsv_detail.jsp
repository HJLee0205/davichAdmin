...
<t:insertDefinition name="davichLayout">
	...
	let promotion = {
		prmtNm: "${prmtNm}",
		prmtNo: "${prmtNo}",
		prmtSDate: new Date(Date.parse("${prmtSDate} 00:00:00")),
		prmtEDate: new Date(Date.parse("${prmtEDate} 23:59:59")),
	}
	...
	let visitPurposeNm = "${visitVO.visitPurposeNm}";
	<%-- 예약 가능 날짜 --%>
	<%-- todo : 임시 조치이면 컨트롤러에서 예약가능 날자 조회 하도록 수정해야 한다. --%>
	let bookingTerm = visitPurposeNm.match(/[0-9]{4}-[0-9]{2}-[0-9]{2}\s?~\s?[0-9]{4}-[0-9]{2}-[0-9]{2}/);
	if (bookingTerm){
		bookingTerm = bookingTerm[0].split('~');
		bookingTerm[0] = new Date(Date.parse(bookingTerm[0].trim() + " 00:00:00"));
		bookingTerm[1] = new Date(Date.parse(bookingTerm[1].trim() + " 23:59:59"));
	}
	...
	function checkPromotionTerm(date, beginDate, endDate, title){
		...
		if (date.isBefore(beginDate) || date.isAfter(endDate)) {
			Dmall.LayerUtil.alert(
					"예약날자는 <br>"+beginDateString+" ~ "+endDateString+"사이의 날짜만 선택 하실 수 있습니다.",
					'확인'
			);
			return false;
		}
        ...
        $('.btn_info_cancel').on('click', function() {
            ...
            Dmall.AjaxUtil.getJSON(url, param, function(result) {
                if(result.success) {
                    ...
                    Dmall.FormUtil.submit('${_MOBILE_PATH}/front/visit/nomember-rsv-list', {'rsvNo':rsvNo, 'rsvMobile':'${so.rsvMobile}', 'nonRsvMobile':'${so.rsvMobile}'});
                }
            });
        });
		...
		$("#btnChgDate").click(function() {
		 ...
		 $(".popup_visit01").click(function() {
				...
				$('.popup_date #calendar').fullCalendar({
						...
						initialDate: '<fmt:formatDate pattern="yyyy-MM-dd" value="${visitVO.rsvDate}" />',
						defaultDate: '<fmt:formatDate pattern="yyyy-MM-dd" value="${visitVO.rsvDate}" />',
						events: eventData,
						dayClick: function(date, allDay, jsEvent, view) {

							<%-- 프로모션 예약 가능날짜 --%>
							if (bookingTerm){
								if(!checkPromotionTerm(date, bookingTerm[0], bookingTerm[1], "")){
									return false;
								}
							}
		...
		Dmall.AjaxUtil.getJSON(url, param, function(result) {
			...
			if ( $('#calendar').children().length > 0 ) {
				...
				if (bookingTerm) {
					...
					let end = new Date(bookingTerm[1]);
					end.setDate(end.getDate() + 1);
					teanseaonDate.push({
						color: "#99CCFF",
						start: bookingTerm[0].format('yyyy-MM-dd'),
		...
		<div class="">
			...
			<ul class="visit_detail_list"><!-- 2018-09-06마크업수정-->
				...
				<li class="form">
					...
					<p class="detail">${visitVO.noMemberNm}</p>
				</li>
				<li class="form">
				...
				<li class="form">
					...
					<p class="detail visit-purpose-nm">
