                ...
                $('#btn_non_visit').on('click',function(){
					...
					if(loginYn != 'true') {
						...
						function() {
							...
							$('#visitForm').find('#returnUrl').val(HTTPS_SERVER_URL+'${_MOBILE_PATH}/front/event/trevues-event');
					...
					Dmall.AjaxUtil.getJSON(url, param, function(result) {
						if(result.success){
							...
							if(data.shareCnt > 0){
								$("#snsShareYn").val('Y');
								$('#visitForm').attr('action','${_MOBILE_PATH}/front/goods/goods-detail?goodsNo=G2104082005_8744');
								$('#visitForm').attr('method','post');
								$('#visitForm').submit();
							}else{
