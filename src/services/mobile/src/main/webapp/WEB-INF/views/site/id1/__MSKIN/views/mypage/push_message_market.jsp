...
<t:insertDefinition name="davichLayout">
    <t:putAttribute name="title">메세지함(마켓)</t:putAttribute>
    ...
    <t:putAttribute name="script">
        ...
        $(document).ready(function(){
    		...
    		$('.more_view').on('click', function() {
				...
				$("#page").val(pageIndex);
             	...
             	var url = '${_MOBILE_PATH}/front/member/push-message-market-paging';
            ...
            $('.btn_form').on('click', function() {
				...
				var startDt = $("#event_start").val().replaceAll('-', '');
				var endDt =$("#event_end").val().replaceAll('-', '');
				if(startDt > endDt){
				    Dmall.LayerUtil.alert('조회 시작일자를 종료일자보다 빠르게 입력해 주세요.','','');
                    return;
				}
                ...
                Dmall.FormUtil.submit('${_MOBILE_PATH}/front/member/push-message-market', param);
            });
        ...
        function fn_push_confirm(obj) {
			...
			var d = $tr.data();
			if(d.readyn == 'N'){
				var param = {pushType:'market',pushNo:d.pushno};
				var url = MOBILE_CONTEXT_PATH+'/front/member/push-message-confirm';
			    Dmall.AjaxUtil.getJSON(url, param, function(result) {
			        if(result.success) {
			        	$tr.removeClass('no_read');
			        	$tr.data('readyn','Y');
			        	$('#newMarketPushCnt').text(Number($('#newMarketPushCnt').text())-1);
			        }
			    });
			}
        }
	...
	<div id="middle_area" class="message">	
		...
		<div class="message_menu">
			<a href="javascript:;" class="active">다비치 마켓<i><em id="newMarketPushCnt">${newMarketPushCnt }</em></i></a>
			<a href="${_MOBILE_PATH}/front/member/push-message">다비치 매장<i><em>${newStorePushCnt }</em></i></a>
		</div>
				...
				<table class="tProduct_Board offline wd-100p my_qna_table02">
					...
					<tbody id="tbody">
						<c:choose>
							<c:when test="${fn:length(resultListModel.resultList) > 0}">
                                	<c:forEach var="li" items="${resultListModel.resultList }">
                               			<tr class="title <c:if test="${li.readYn eq 'N' }">no_read</c:if>" data-readyn="${li.readYn }" data-pushno="${li.pushNo }" onClick="javascript:fn_push_confirm(this);">
                               				<td>${li.appDate }</td>
                                    		<td style="text-align:left;padding-left:10px;">${fn:substring(li.sendMsg,0,30)}</td>
                                    		<td class="textC">${li.appTime }</td>
                                    		<%--<td class="textC">다비치마켓</td>--%>
                                    	</tr>
                                    	<tr class="hide">
											<td colspan="3">
												${li.sendMsg}
							...
							<c:otherwise>
								<tr>
                                    <th class="textC" colspan="3">수신받은 메세지가 없습니다.</th>
                                </tr>
							</c:otherwise>
						</c:choose>
					</tbody>
