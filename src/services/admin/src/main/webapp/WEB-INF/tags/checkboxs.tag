...
<%@ attribute name="valueList" required="false" description="체크박스 태그의 다중 값" %>
<%
    int index = 1;
    String input;
    String id;
    ...
    StringTokenizer valueListToken = null;
    if(valueList != null && valueList != "") {
        valueListToken = new StringTokenizer(valueList, ";");
    }
    String valueToken;
    ...
    while(codeListToken.hasMoreTokens()) {
        ...
        if(valueListToken != null && valueListToken.hasMoreTokens()) {
            valueToken = valueListToken.nextToken();
            value = valueToken;
        } else {
            value = null;
        }
