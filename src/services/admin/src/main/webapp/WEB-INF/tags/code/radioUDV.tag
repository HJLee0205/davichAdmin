...
<%@ attribute name="check" required="false" description="라디오 태그의 checked 처리 여부" type="java.lang.Boolean" %>
<%
    int index = 1;
    ...
    for(CmnCdDtlVO codeDetail : codesCopy) {
        ...
        if ((check == null || check == true ) && value == null && index == 1 || codeDetail.getDtlCd().equals(value)) {
...
%>
<label for="${idPrefix}_<%= index %>" class="radio mr20<%= on %>">
    <span class="ico_comm"><input type="radio" name="${name}" id="${idPrefix}_<%= index++ %>"<%= checked %> value="<%= codeDetail.getDtlCd() %>"<%= validate %> /></span><%= codeDetail.getDtlNm() %>
</label>
<%
    }
%>