...
<t:insertDefinition name="defaultLayout">
    <t:putAttribute name="title">홈 &gt; 프로모션 &gt; 기획전 &gt; 기획전 수정</t:putAttribute>
        ...
        <script type="text/javascript">
			$(document).ready(function () {
				...
				$('#input_id_image').on('change', function (e) {
					...
					if($('.upload_file').children().is('button')) {
						Dmall.LayerUtil.alert('이미 이미지가 등록되어 있습니다.<br/>기존 이미지를 삭제 후 진행해주세요.');
						return;
					}
			...
			var ExhibitionUtil = {
				init: function () {
					...
					Dmall.DaumEditor.create('prmtContentHtml');

					Dmall.DaumEditor.setContent('prmtContentHtml', $('#hd_prmtContentHtml').val());
					Dmall.DaumEditor.setAttachedImage('prmtContentHtml', JSON.parse('${attachImages}'));

					Dmall.validate.set('formExhibitionUpdate');

					//qrcode
					$('#qrcodeCanvas').qrcode({
						width: 100,
						height: 100,
						text: "http://www.davichmarket.com/m/front/qrcode?prmtNo=${resultModel.data.prmtNo}"
					});
				},
				validation: function() {
					if( $("#goodsTypeCd").val() == ""){
				...
				save: function () {
					...
					if(($('#prePrmtDcValue').val() === $('#prmtDcValue').val())
						|| $('#prmtDcValue').val() == "0"
							|| $('#prmtDcValue').val() == "") {
						if (Dmall.validate.isValid('formExhibitionUpdate')) {
							Dmall.LayerUtil.confirm('저장하시겠습니까?', function () {
								Dmall.DaumEditor.setValueToTextarea('prmtContentHtml');

								var url = '/admin/promotion/exhibition-update';

								$('#formExhibitionUpdate').ajaxSubmit({
									url: url,
									dataType: 'json',
						...
						var goodsNoArr = fn_selectedList();
						var param = {goodsNos: goodsNoArr, prmtNo: $('#prmtNo').val()};

						console.log("param = ", param);
						Dmall.AjaxUtil.getJSON(url, param, function (result) {
							if (result != null && result > 0) {
								Dmall.LayerUtil.alert("프로모션이 할인율이 적용 되고 있는 상품입니다.");
								return;
							}
							if (Dmall.validate.isValid('formExhibitionUpdate')) {
								Dmall.LayerUtil.confirm('저장하시겠습니까?', function () {
				...
				download: function () {
					...
					$(div).qrcode({
						...
						text: 'http://www.davichmarket.com/front/qrcode?goodsNo=${resultModel.data.prmtNo}'
					});
			...
			var PopupUtil = {
				...
				applyCallback: function (data) {
					...
					if( $("#prmtDcValue").val() != "" && $("#prmtDcValue").val() != "0"){
						...
						Dmall.AjaxUtil.getJSON(url, param, function(result) {
							if (result != null && result > 0) {
								Dmall.LayerUtil.alert("프로모션 할인율이 적용 되고 있는 상품입니다.");
								return;
							}
    ...
    <t:putAttribute name="content">
		...
		<c:set var="goodsList" value="${prmtDtl.goodsList}" />
       	<div class="sec01_box">
			<input type="hidden" id="hd_prmtContentHtml" value="${prmtDtl.prmtContentHtml}">
            <div class="tlt_box">
				<div class="tlt_box">
					<div class="tlt_head">
            ...
            <form id="formExhibitionUpdate" method="post">
            	<input type="hidden" name="prmtNo" id="prmtNo" value="${prmtDtl.prmtNo}" />
                <input type="hidden" name="prmtWebBannerImgPath" id="prmtWebBannerImgPath" value="${prmtDtl.prmtWebBannerImgPath}"/>
                <input type="hidden" name="prmtWebBannerImgOrg" id="prmtWebBannerImgOrg" value="${prmtDtl.dlgtImgOrgNm}"/>
                <input type="hidden" name="prmtWebBannerImg" id="prmtWebBannerImg" value="${prmtDtl.prmtWebBannerImg}"/>
                <%--<input type="hidden" name="prmtMobileBannerImgPath" id="prmtMobileBannerImgPath" value="${prmtDtl.prmtMobileBannerImgPath}"/>
                <input type="hidden" name="prmtMobileBannerImgOrg" id="prmtMobileBannerImgOrg" value="${prmtDtl.prmtMobileBannerImg}"/>
                <input type="hidden" name="prmtMobileBannerImg" id="prmtMobileBannerImg" value="${prmtDtl.prmtMobileBannerImg}"/>--%>
                <input type="hidden" id="applyEndDttmOrg" value="${prmtDtl.applyEndDttm}"/>  <!-- 수정 전 유효기간 종료일시 : 폼으로 넘기지 않고 스크립트에서만 사용 -->
                    ...
                    <div class="tblw tblmany">
                    	<table summary="이표는 기획전  검색 표 입니다. 구성은 기획전명, 설명, 기간, 내용, 웹전용 기획전배너, 모바일배너, 기획전 상품설정 입니다.">
								...
								<td>
									<span class="select">
										...
										<select name="goodsTypeCd" id="goodsTypeCd">
											<tags:option codeStr=":선택;01:안경테;02:선글라스;04:콘텍트렌즈;03:안경렌즈" value="${prmtDtl.goodsTypeCd}" />
										</select>
									</span>
								</td>
							</tr>
								...
								<td>
									<span class="select">
										...
										<select name="prmtTypeCd" id="prmtTypeCd">
											<tags:option codeStr="01:혜택;02:쿠폰;03:사은품;04:세일;06:첫 구매 특가" value="${prmtDtl.prmtTypeCd}"/>
										</select>
									</span>
								</td>
							</tr>
							<tr>
								<th>기획전 명</th>
								<td>
									<span class="intxt wid100p"><input type="text" value="${prmtDtl.prmtNm}" id="prmtNm" name="prmtNm" data-validation-engine="validate[required, maxSize[100]]"/></span>
								</td>
							</tr>
							<tr>
								<th>기획전 설명</th>
								<td>
									<span class="intxt wid100p"><input type="text" value="${prmtDtl.prmtDscrt}" id="prmtDscrt"  name="prmtDscrt" data-validation-engine="validate[required, maxSize[300]]"/></span>
								</td>
							</tr>
							<tr>
								<th>SEO 검색용 태그달기</th>
								<td>
									 <div class="txt_area">
										<textarea name="seoSearchWord" id="txt_seo_search_word" data-validation-engine="validate[maxSize[2500]]">${prmtDtl.seoSearchWord}</textarea>
									</div>
									<span class="br2"></span>
							...
							<tr>
								<th>기획전 기간</th>
								<td>
									<tags:calendarTime from="from" to="to" idPrefix="srch" fromValue="${prmtDtl.applyStartDttm}" toValue="${prmtDtl.applyEndDttm}"/>
								</td>
							</tr>
							<tr>
								<th>기획전 내용</th>
								<td>
									<div class="edit">
										<textarea id="prmtContentHtml" class="blind"  name="prmtContentHtml" maxlength="8000"></textarea>
									</div>
								</td>
							</tr>
							<tr>
								<th>기획전 대표 이미지</th>
								<td>
									<span class="intxt imgup2"><input type="text" id="file_route1" class="upload-name" readonly value=""></span>
									...
									<div class="upload_file">
										<c:if test="${fn:length(prmtDtl.prmtWebBannerImg) gt 1}">
											<img src="${_IMAGE_DOMAIN}/image/image-view?type=EXHIBITION&path=${prmtDtl.prmtWebBannerImgPath}&id1=${prmtDtl.prmtWebBannerImg}" alt="">
											<br>
											<span class="txt">${prmtDtl.prmtWebBannerImg}</span>
											<button class="cancel">삭제</button>
										</c:if>
									</div>
								</td>
							...
							<tr>
								<th>기획전 상품 설정</th>
								<td id="goods_search_empty" <c:if test="${goodsList ne null and goodsList.size() gt 0}">style="display: none;"</c:if> >
									<a href="#none" class="btn--black_small btn_goods_popup">상품 찾기</a>
								</td>
								<td id="goods_search_exist" <c:if test="${goodsList eq null or goodsList.size() eq 0}">style="display: none;"</c:if> >
									<div class="top_lay">
									...
									<div class="tblh h600">
										<table summary="이표는 판매상품관리 리스트 표 입니다. 구성은 선택, 번호, 카테고리, 상품명, 판매가, 재고/가용, 배송비, 판매상태, 전시상태, 관리 입니다.">
											...
											<c:forEach var="goods" items="${goodsList}" varStatus="status">
												<tr>
													<td>
														<input type="hidden" name="goodsNoArr" value="${goods.goodsNo}">
														<label for="chk_goodsNo_${goods.rownum}" class="chack"><span class="ico_comm">
															<input type="checkbox" id="chk_goodsNo_${goods.rownum}" class="blind">
														</span></label>
													</td>
													<td>${goods.rownum}</td>
													<td><img src="${_IMAGE_DOMAIN}${goods.imgPath}" alt=""></td>
													<td>${goods.goodsNm}</td>
													<td>${goods.goodsNo}</td>
													<td>${goods.brandNm}</td>
													<td>${goods.sellerNm}</td>
													<td><fmt:formatNumber value="${goods.salePrice}" pattern="#,###" /></td>
													<td><fmt:formatNumber value="${goods.stockQtt}" pattern="#,###" /></td>
													<td>${goods.goodsSaleStatusNm}</td>
													<td>${goods.erpItmCode}</td>
												</tr>
											</c:forEach>
											</tbody>
										</table>
							...
							<tr>
								<th>기획전 상품가격</th>
								<td>
									<tags:radio name="prmtDcGbCd" codeStr="01:할인율;02:할인금액" idPrefix="prmtDcGbCd" value="${prmtDtl.prmtDcGbCd}"/>
									<span class="intxt shot">
										<input type="text" name="prmtDcValue" id="prmtDcValue" value="${prmtDtl.prmtDcValue}" data-validation-engine="validate[required, maxSize[20]]" style="width:65px;">
									</span>
									<span id="dcValueSpan" class="marginL05"><c:if test='${prmtDtl.prmtDcGbCd eq "01"}'>%</c:if><c:if test='${prmtDtl.prmtDcGbCd eq "02"}'>원</c:if></span>
								</td>
							</tr>
							<tr>
								<th>본사 부담율</th>
								<td>
									<input type="number" name="prmtLoadrate" id="prmtLoadrate" data-validation-engine="validate[required, maxSize[3]]" value="${prmtDtl.prmtLoadrate}" style="width:80px;">
							...
							<tr>
								<th>게시물 노출 여부</th>
								<td>
									<tags:radio name="prmtMainExpsUseYn" codeStr="Y:사용;N:미사용" idPrefix="prmtMainExpsUseYn" value="${prmtDtl.prmtMainExpsUseYn}"/>
								</td>
							</tr>
							<tr id="firstBuySpcPrice_layer" style="display: none;">
								<th>첫구매 특가</th>
								<td>
									<input type="number" name="firstBuySpcPrice" id="firstBuySpcPrice" value="${prmtDtl.firstBuySpcPrice}" data-validation-engine="validate[required, maxSize[20]]" style="width:80px;">
								</td>
							</tr>
							<tr>
								<th>QR코드</th>
