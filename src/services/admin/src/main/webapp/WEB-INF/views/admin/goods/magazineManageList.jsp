...
<t:insertDefinition name="defaultLayout">
    <t:putAttribute name="title">매거진관리</t:putAttribute>
    <t:putAttribute name="script">
        <script>
            ...
            function loadDefaultCondition() {
                var cookie =  getCookie('SEARCH_MAGAZINE_LIST');

                if (!cookie) {
                    return;
                } else {
                    // var cookieObj =  JSON.parse(getCookie('SEARCH_MAGAZINE_LIST'));
                    var cookieObj =  jQuery.parseJSON(getCookie('SEARCH_MAGAZINE_LIST'));
                    if (cookieObj['page']) {
            ...
            function fn_callback_pop_goods(data) {
                alert('매거진 선택 팝업 리턴 결과 :' + data["goodsNo"] + ', data :' + JSON.stringify(data));
            }

            // 매거진 등록 팝업 호출
            function fn_registGoods() {
                // alert('미구현 기능 - 매거진등록 (매거진상세화면)');
                location.href= "/admin/goods/magazine-detail";
            }
            ...
            function fn_delete() {
                ...
                if (selected) {
                    Dmall.LayerUtil.confirm('선택된 매거진을 삭제 하시겠습니까?', function() {
            ...
            function fn_soldout() {
                var selected = fn_selectedList();
                if (selected) {
                    Dmall.LayerUtil.confirm('매거진의 판매 상태를 품절로 변경하시겠습니까?', function() {
            ...
            function fn_display() {
                var selected = fn_selectedList();
                if (selected) {
                    Dmall.LayerUtil.confirm('매거진의 전시 상태를 전시로 변경하시겠습니까?', function() {
            ...
            function fn_noDisplay() {
                var selected = fn_selectedList();
                if (selected) {
                    Dmall.LayerUtil.confirm('매거진의 전시 상태를 미전시로 변경하시겠습니까?', function() {
                        var url = '/admin/goods/display-update',
                            param = {},
                            key;
            ...
            function fn_salestop() {
                var selected = fn_selectedList();
                if (selected) {
                    Dmall.LayerUtil.confirm('매거진의 판매 상태를 판매중지로 변경하시겠습니까?', function() {
            ...
            function fn_salestart() {
                var selected = fn_selectedList();
                if (selected) {
                    Dmall.LayerUtil.confirm('매거진의 판매 상태를 판매중으로 변경하시겠습니까?', function() {
            ...
            function fn_save() {
                var selected = fn_selectedList();
                if (selected) {
                    Dmall.LayerUtil.confirm('수정한 매거진 정보를 저장하시겠습니까?', function() {
            ...
            function getSearchGoodsData() {
                ...
                setCookie('SEARCH_MAGAZINE_LIST', JSON.stringify($('#form_id_search').serializeObject()), expdate);
                ...
                $.each( ctgArr, function( index, value ){
                	if(dlgtCtgArr != null && ctgCmsRateArr != null){
                		if(dlgtCtgArr.length >= index && ctgCmsRateArr.length >= index){
                            html +=  value +(dlgtCtgArr[index]=="Y"?"":"") +"<BR>" ;
                		}
                	}
                });
                obj.html(html);
            ...
            function setSalePrice(data, obj, bindName, target, area, row) {
                ...
                obj.on('keyup', function(e) {
                    ...
                    // 공급가 변경... (별도 공급가 적용 매거진이 아닐경우만 변경 가능)
            ...
            function setSupplyPrice(data, obj, bindName, target, area, row) {
                ...
                obj.val(supplyPrice);
                obj.after("<div>"+numberWithCommas(supplyPrice)+"<br><b>"+commisionRate+"</b></div>("+numberWithCommas(goodseachDlvrc)+")");
            }
            ...
            function executeGoodsDetail(obj) {
                ...
                $targetTr = $("#" + trID);
                location.href= "/admin/goods/magazine-detail-edit?goodsNo=" + obj.data('goodsNo');
            }
            ...
            function executeGoodsCopy(obj) {
                var trID = "tr_goods_" + obj.data("goodsNo");
                // alert("복사클릭 (복사본 매거진 등록 필요 - 해당 기능 미구현, 매거진등록 구현 이후 처리필요):" + trID);
                // $targetTr = $("#" + trID);

                Dmall.LayerUtil.confirm('매거진 정보를 복사하시겠습니까?', function() {
    ...
    <t:putAttribute name="content">
        <div class="sec01_box">
            <div class="tlt_box">
                <h2 class="tlth2">D.매거진 관리 </h2>
                <div class="btn_box right">
                    <a href="javascript:;" class="btn blue"  id="btn_regist">매거진등록</a>
                </div>

            </div>
        <!-- search_box -->
        <form:form id="form_id_search" >
        <input type="hidden" name="goodsContsGbCd" id="goodsContsGbCd" value="02" />
        ...
        <div class="search_box">
            ...
            <div class="search_tbl">
                <table summary="이표는 D.매거진관리 검색 표 입니다. 구성은 카테고리, 브랜드, 기간, 매거진가격, 판매상태, 전시상태, 판매자, 검색어 입니다.">
                    <caption>D.매거진관리 검색</caption>
                    <colgroup>
                        <col width="10%">
                        <col width="">
                    </colgroup>
                    <tbody>
                        ...
                        <%--<tr>
                            <th>매거진가격</th>
                            <td>
                                <span class="intxt"><input type="text" name="searchPriceFrom" id="txt_search_price_from" class="txtr comma" maxlength="10" style="width:106px;"/></span> 원
                                ~
                                <span class="intxt"><input type="text" name="searchPriceTo" id="txt_search_price_to" class="txtr comma" maxlength="10" style="width:106px;"/></span> 원
                            </td>
                        </tr>--%>
                        <%--<tr>
                            <th>판매상태</th>
                            <td id="td_goodsStatus">
                                ...
                                &lt;%&ndash;<a href="#none" id="goodsStatus" class="all_choice"><span class="ico_comm"></span> 전체</a>&ndash;%&gt;
                            </td>
                        </tr>--%>
                        <tr>
                            <th>전시상태</th>
        ...
        <div class="line_box">
            ...
            <!-- tblh -->
            <div class="tblh">
                    <table style="table-layout:fixed;" summary="이표는 D.매거진관리 리스트 표 입니다. 구성은 선택, 번호, 카테고리, 매거진명, 판매가, 재고/가용, 배송비, 판매상태, 전시상태, 관리 입니다.">
                        <caption>D.매거진관리 리스트</caption>
                        <colgroup>
                            <col width="3%">
                            <col width="3%">
							<col width="120px">
                        ...
                        <tbody id="tbody_goods_data">
                            <tr id="tr_goods_data_template" >
                                ...
                                <%--<td data-bind="goodsInfo" data-bind-type="string" data-bind-value="goodsNo">매거진코드</td>--%>
                                ...
                                <td>
                                    <span class="intxt">
                                        <input type="text" class="comma" name="salePrice" data-bind="goodsInfo" data-bind-type="function" data-bind-function="setSalePrice"  data-bind-value="salePrice" data-validation-engine="validate[required, maxSize[10]]" maxlength="10">&lt;%&ndash;판매가&ndash;%&gt;
                                ...
                                <td>
                                    <span class="intxt shot">
                                        <input type="hidden" class="comma" name="supplyPrice" data-bind="goodsInfo" data-bind-type="function" data-bind-function="setSupplyPrice"  data-bind-value="supplyPrice" data-validation-engine="validate[maxSize[10]]" maxlength="10" readonly>&lt;%&ndash;공급가&ndash;%&gt;
                                    </span>
                                </td>
                                ...
                                <td class="txtc" data-bind="goodsInfo" data-bind-type="string" data-bind-value="goodsInqCnt"></td>
                                <td data-bind="goodsInfo" data-bind-value="goodsNo" data-bind-type="function" data-bind-function="setGoodsDisplayText"></td>
                                <td>
                            ...
                            <tr id="tr_no_goods_data"><td colspan="8">데이터가 없습니다.</td></tr>
                        </tbody>
                    </table>
            </div>
            <!-- //tblh -->
             ...
             <div class="bottom_lay">
                ...
                <%--<div class="right">
                    <div class="pop_btn">
                        ...
                        <!-- <button class="btn_gray2"  id="btn_pop_goods">- 매거진검색 팝업 - 삭제대상</button> -->
                    </div>
                </div>--%>
