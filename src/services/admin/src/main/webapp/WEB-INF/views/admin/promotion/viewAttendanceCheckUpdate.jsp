...
<t:insertDefinition name="defaultLayout">
    <t:putAttribute name="title">홈 &gt; 프로모션 &gt; 출석체크 이벤트 수정</t:putAttribute>
    <t:putAttribute name="script">
        ...
        var $pointSrch_to_minute = $("#pointSrch_to_minute");
            ...
            jQuery(document).ready(function() {
                ...
                //이벤트목록 페이지로 돌아가기 : 기간검색옵션,검색시작일,검색종료일,이벤트진행상태,검색어,노출개수,페이지번호
                ...
                jQuery('#reg_btn').on('click', function(e) {
                    ...
                    if(Dmall.validate.isValid('form_info')) {
                        var url = '/admin/promotion/attendance-check-update',
            ...
            function dttmCheck(){
                ...
                // 다른 이벤트( 시작일시/ 종료일시/ 이벤트이름/ 이벤트번호 )
                var otherEventStartDttm = $('#otherEventDttm').children('input:nth-child(4n+1)');
                var otherEventEndDttm = $('#otherEventDttm').children('input:nth-child(4n+2)');
                var otherEventNm = $('#otherEventDttm').children('input:nth-child(4n+3)');
                var otherEventNo = $('#otherEventDttm').children('input:nth-child(4n)');
                ...
                for( var j = 0; j < otherEventEndDttm.length; j++ ){
                    if( otherEventStartDttm.eq(j).prop('value') <=  currentEventEndDttm ) {
                        if( otherEventEndDttm.eq(j).prop('value') >=  currentEventStartDttm ) {
                            // 다른 이벤트에 만들려는 이벤트도 들어있다. 그래서 이벤트번호가 같은 경우는 중복체크 안한다.
                            if( otherEventNo.eq(j).prop('value') !=  $("#eventNo").prop('value') ) {
...
//                 if($pointSrch_sc01.val() == '' || $pointSrch_sc01.val() == null || $pointSrch_sc02.val() == '' || $pointSrch_sc02.val() == null ) {
    ...
    <div class="sec01_box">
        ...
        <!--         <input type="text" id="pointPvdYn" value="" />  -->
                ...
                <table summary="이표는 이벤트등록표 입니다. 구성은 출석체크명, 종류, 체크기간, 예외설정, 출석체크 형태, 참여방법, 출석체크 참여혜택, 참여권한설정 입니다.">
                    ...
                    <tbody>
                        <tr>
                            <th>출석체크명</th>
                            <td>
                                <span class="intxt wid100p"><input type="text" id="eventNm" name="eventNm" value="${eventVO.eventNm}" /></span>
                            </td>
                        </tr>
                        <tr>
                            <th>참여방법</th>
                            <td>
                                <label for="loginType" class="radio mr20<c:if test='${eventVO.eventMethodCd eq 01}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventMethodCd" id="loginType" value="01" <c:if test='${eventVO.eventMethodCd eq 01}'> checked="checked" </c:if> /></span> 로그인형 <span class="fc_pr1 fs_pr1">(로그인 시 자동 반영)</span></label>
                                <label for="stampType" class="radio mr20<c:if test='${eventVO.eventMethodCd eq 02}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventMethodCd" id="stampType" value="02" <c:if test='${eventVO.eventMethodCd eq 02}'> checked="checked" </c:if> /></span> 스템프형<span class="fc_pr1 fs_pr1">(출석체크 버튼 클릭 시 반영)</span></label>
                            </td>
                        </tr>
                        <tr>
                            <th>출석체크 기간</th>
                        ...
                        <tr>
                            <th>기간 예외설정</th>
                            <td>
                                <label for="exceptSat" class="radio mr20<c:if test='${eventVO.eventPeriodExptCd eq 01}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventPeriodExptCd" id="exceptSat" value="01" <c:if test='${eventVO.eventPeriodExptCd eq 01}'> checked="checked"</c:if>/></span> 토요일제외</label>
                                <label for="exceptSun" class="radio mr20<c:if test='${eventVO.eventPeriodExptCd eq 02}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventPeriodExptCd" id="exceptSun" value="02" <c:if test='${eventVO.eventPeriodExptCd eq 02}'> checked="checked"</c:if>/></span> 일요일제외</label>
                                <label for="exceptNone" class="radio mr20<c:if test='${eventVO.eventPeriodExptCd eq 03}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventPeriodExptCd" id="exceptNone" value="03" <c:if test='${eventVO.eventPeriodExptCd eq 03}'> checked="checked"</c:if>/></span> 설정안함</label>
                            </td>
                        </tr>
                        <tr>
                            <th>출석 포인트 지급</th>
                            <td>
                                <span class="intxt"><input type="text" name="eventPvdPoint" id="eventPvdPoint" value="${eventVO.eventPvdPoint}"/></span> 점 <span class="fc_pr1 fs_pr1">(1일 최대 지급 포인트를 의미함)</span>
                            </td>
                        </tr>
                        <tr>
                            <th>출석 포인트<br>유효기간 설정</th>
                            <td>
                                <label for="fromToPeriod" class="radio mr20<c:if test='${eventVO.eventPointApplyCd eq 01}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventPointApplyCd" id="fromToPeriod" value="01" <c:if test='${eventVO.eventPointApplyCd eq 01}'> checked="checked"</c:if>/></span> </label>
<%--                                 <tags:calendarTime from="pointFrom" to="pointTo" idPrefix="pointSrch" fromValue="${eventVO.pointApplyStartDttm}" toValue="${eventVO.pointApplyEndDttm}" /> --%>
                                ...
                                <span class="br2"></span>
                                <label for="issueAfPeriod" class="radio mr20<c:if test='${eventVO.eventPointApplyCd eq 02}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventPointApplyCd" id="issueAfPeriod" value="02" <c:if test='${eventVO.eventPointApplyCd eq 02}'> checked="checked"</c:if>/></span> </label>
                                적립일로부터 <span class="intxt"><input type="text" name="eventApplyIssueAfPeriod" id="eventApplyIssueAfPeriod" <c:if test='${eventVO.eventPointApplyCd eq 02}'> value="${eventVO.eventApplyIssueAfPeriod}" </c:if>/></span> 일 유효(사용기한 초과 포인트 자동 소멸)
                            </td>
                        </tr>
                        <tr>
                            <th>출석 이벤트 조건</th>
                            <td>
                                <div class="txtind_box">
                                    <label for="cndtMeet" class="radio left<c:if test='${eventVO.eventCndtCd eq 01}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventCndtCd" id="cndtMeet" value="01" <c:if test='${eventVO.eventCndtCd eq 01}'> checked="checked"</c:if>/></span> 조건완성형:</label>
                                    <div class="right">
                                        총 참여횟수가 <span class="intxt shot"><input type="text" name="eventTotPartdtCndt01" id="eventTotPartdtCndt01" <c:if test='${eventVO.eventCndtCd eq 01}'> value="${eventVO.eventTotPartdtCndt}" </c:if> /></span> 회 이상일 경우 이벤트 조건을 만족합니다.
                                    </div>
                                    <span class="br"></span>
                                    <label for="extraPvd" class="radio left<c:if test='${eventVO.eventCndtCd eq 02}'> on</c:if>"><span class="ico_comm"><input type="radio" name="eventCndtCd" id="extraPvd" value="02" <c:if test='${eventVO.eventCndtCd eq 02}'> checked="checked"</c:if>/></span> 추가지급형:</label>
                                    <div class="right">
                                        총 참여횟수 <span class="intxt shot"><input type="text" name="eventTotPartdtCndt02" id="eventTotPartdtCndt02" <c:if test='${eventVO.eventCndtCd eq 02}'> value="${eventVO.eventTotPartdtCndt}" </c:if> /></span> 회 만족 시 추가포인트를 지급합니다.
                                        <span class="br2"></span>
                                        포인트 <span class="intxt shot"><input type="text" id="eventAddPvdPoint" name="eventAddPvdPoint" <c:if test='${eventVO.eventCndtCd eq 02}'> value="${eventVO.eventAddPvdPoint}" </c:if> />  </span> 점 추가지급
                                    </div>
                                </div>
                            </td>
                        </tr>
            ...
            <span id="otherEventDttm">
                <c:forEach var = "result" items = "${list.resultList}">
                        ...
                        <input type="hidden" value="${result.otherEventNo}" />
                </c:forEach>
            </span>
