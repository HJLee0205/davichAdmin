...
<c:if test="${typeCd eq '03'}">
    <c:set var="filterTypeCd" value="lensFilter"/>
</c:if>
<c:if test="${typeCd eq '04'}">
    <c:set var="filterTypeCd" value="contFilter"/>
</c:if>
<c:if test="${typeCd eq '05'}">
    <c:set var="filterTypeCd" value="etcFilter"/>
</c:if>
    ...
    <t:putAttribute name="script">
        ...
        <script>
            jQuery(document).ready(function () {
                        ...
                        $.ajax({
                            ...
                            if (result) {
                                ...
                                if (result == null || result.success != true) {
                                    ...
                                    <%--Dmall.FormUtil.submit("/admin/goods/sales-item?typeCd=${typeCd}");--%>
                ...
                // 재고 다비전 연동 체크
                $('#lb_applyDavisionStockYn').off('click').on('click', function (e) {
                    Dmall.EventUtil.stopAnchorAction(e);
                    ...
                    if($input.prop('checked')) {
                        $('#txt_stock_qtt').prop('readonly', true);
                        $this.addClass('on');
                    } else {
                        $('#txt_stock_qtt').prop('readonly', false);
                        $this.removeClass('on');
                    }
                });

                // 물류재고 연동 (다중옵션)
                $('#lb_multiDavisionStockYn').off('click').on('click', function (e) {
                    Dmall.EventUtil.stopAnchorAction(e);
                ...
                $("#sel_seller").off("change").on("change", function () {
                    ...
                    var sellerNo = $("#sel_seller option:selected").val();
                    ...
                    if (sellerNo == "1") {
                        ...
                        $("#tr_no_goods_item td").attr("colspan", "10"); // 데이터 없는 경우 colspan변경
                        $('#lb_applyDavisionStockYn').removeClass("forceHide"); // 재고 다비전 연동 (단일옵션)
                        $('#lb_multiDavisionStockYn').removeClass('forceHide'); // 물류재고 연동 (다중옵션)
                    } else {
                        // 판매자가 다비치가 아닌 경우
                        // 다비젼 상품 코드 설정 부분 숨기기
                        ...
                        $("#tr_no_goods_item td").attr("colspan", "9"); // 데이터 없는 경우 colspan변경
                        $('#lb_applyDavisionStockYn').addClass("forceHide");    // 재고 다비전 연동 (단일옵션)
                        $('#lb_multiDavisionStockYn').addClass('forceHide'); // 물류재고 연동 (다중옵션)
                    }
                });
                ...
                jQuery("#btn_erpItm").on("click", function () {
                    erpItemSearchPop(jQuery("#txt_erp_itm_code"), 'S');
                });
                // 라디오 버튼 변경시 다른 애들은 disabled시키자
                jQuery("input[name=erp_itm_srch_type]").off("change").on("change", function () {
                    // 일단 전부다 disabled 시키고
                    $(this).closest("tbody").find("select,input[type!=radio]").prop("disabled", true);
                    $(this).closest("tbody").find("span.select").addClass("disa");
                    // 자기쪽 것만 disabled 해제
                    $(this).closest("tr").find("select,input[type!=radio]").prop("disabled", false);
                ...
                jQuery("#apply_erp_itm_code").off("click").on("click", function () {
                    ...
                    if($('#layer_erp_itm_code').data('option-type') == 'S') {
                        ...
                        if(${typeCd ne '04'}) {
                            ...
                            $.each(filterInfo, function (idx, obj) {
                                if(obj.indexOf('contOptCode_') != -1) {
                                    if(obj.split('_')[1] == '01') {
                                        $('#sel_filter_type').find('option:contains("투명")').prop('selected', true);
                                    } else {
                                        $('#sel_filter_type').find('option:contains("컬러")').prop('selected', true);
                                    }
                                    changeConFilterOptionValue($('#sel_filter_type').val()).then(function () {
                                        loadDavisionFilterInfo(filterMap);
                                    });
                                }
                            });
                        ...
                        if(basicInfo['salePrice'] != basicInfo['supplyPrice']) {
                            $('#lb_sepSupplyPriceYn').trigger('click');
                        }

                        $('#lb_applyDavisionStockYn').addClass('on').find('input').prop('checked', true);
                        $('#txt_stock_qtt').prop('readonly', true);
                    } else if($('#layer_erp_itm_code').data('option-type') == 'M') {
                        var basicInfo = [];
                        var $layerErpItmCode = $("#layer_erp_itm_code [name=erpItmCode]:checked");
                        $layerErpItmCode.closest('span').siblings('input[name^="basicDavi"]').each(function () {
                            var key = $(this).prop("id");
                        ...
                        $(erpItmCodeTarget).closest('tr').find('input[data-bind=item_info]').not('[data-bind-value^=attrValue]').DataBinder(basicInfo);
                        $(erpItmCodeTarget).val($layerErpItmCode.val());

                        $('#lb_multiDavisionStockYn').addClass('on').find('input').prop('checked', true);
                    }
                ...
                $('#btn_download').on('click', function (e) {
                    ...
                    if (null == $('th:visible', '#tr_pop_goods_item_head_2').eq(0).data("optNm")) {
                        Dmall.LayerUtil.alert('옵션 정보를 생성해 주세요');
                        return;
                    }

                    var url = '/admin/goods/item-excel-download',
                        param = {};

                    // optData set
                    var $tr = $('thead', $('#tb_goods_option')).find('tr').last();
                    $tr.find('th').each(function (idx, obj) {
                        param['optData['+ idx +'].optNm'] = $(this).text();
                    });

                    // itmData set
                    var isStandardPriceYn = false;
                    $('tr.itemoption:visible', $('#tbody_item')).each(function (idx, obj) {
                        if($('label', $(this)).eq(0).hasClass('on')) {
                            isStandardPriceYn = true;
                        }
                        param['itmData['+ idx +'].standardPriceYn'] = $('label', $(this)).eq(0).hasClass('on') ? 'Y' : 'N';
                        param['itmData['+ idx +'].optValue1'] = $('input:text[name=attrValue1]', $(this)).val();
                        param['itmData['+ idx +'].optValue2'] = $('input:text[name=attrValue2]', $(this)).val();
                        param['itmData['+ idx +'].optValue3'] = $('input:text[name=attrValue3]', $(this)).val();
                        param['itmData['+ idx +'].optValue4'] = $('input:text[name=attrValue4]', $(this)).val();
                        param['itmData['+ idx +'].cost'] = $('input:text[name=cost]', $(this)).val().trim().replaceAll(',','');
                        param['itmData['+ idx +'].customerPrice'] = $("input:text[name=customerPrice]", $(this)).val().trim().replaceAll(',', '');
                        param['itmData['+ idx +'].salePrice'] = $("input:text[name=salePrice]", $(this)).val().trim().replaceAll(',', '');
                        param['itmData['+ idx +'].supplyPrice'] = $("input:text[name=supplyPrice]", $(this)).val().trim().replaceAll(',', '');
                        param['itmData['+ idx +'].stockQtt'] = $("input:text[name=stockQtt]", $(this)).val().trim().replaceAll(',', '');
                        param['itmData['+ idx +'].erpItmCode'] = $("input:text[name=itemErpItmCode]", $(this)).val().trim();
                    });

                    if(!isStandardPriceYn) {
                        param['itmData[0].standardPriceYn'] = 'Y';
                    }
                ...
                $('#btn_upload').on('click', function (e) {
                    ...
                    $('#input_id_excel').trigger('click');
                });

                $('#input_id_excel').on('change', function () {
                    if($(this).val()) {
                        Dmall.LayerUtil.confirm('해당 엑셀로 옵션을 생성하시겠습니까?'
                            , function() {
                                var url = '/admin/goods/item-excel-upload';

                                Dmall.waiting.start();
                                $('#form_excel_upload').ajaxSubmit({
                                    url: url,
                                    dataType: 'json',
            ...
            function setExcelData(data) {
                ...
                $('tr.itemoption', $('#tbody_item')).each(function (idx, obj) {
                    var data = {
                        ...
                        optValue1 : $("input:text[name=attrValue1]", $(this)).val(),
                        optValue2 : $("input:text[name=attrValue2]", $(this)).val(),
                        optValue3 : $("input:text[name=attrValue3]", $(this)).val(),
                        optValue4 : $("input:text[name=attrValue4]", $(this)).val(),
                        cost : $('input:text[name=cost]', $(this)).val().trim().replaceAll(',',''),
                ...
                // 엑셀의 정보와 비교
                var uniqueItems = [];
                // 엑셀데이터 순회하면서 기존데이터에 없는 것 배열에 추가
                for(const item1 of data) {
                    let isDuplicate = false;
                    for(const item2 of oldData) {
                        if(item1.optValue1 == item2.optValue1 &&
                            item1.optValue2 == item2.optValue2 &&
                            item1.optValue3 == item2.optValue3 &&
                            item1.optValue4 == item2.optValue4) {
                            isDuplicate = true;
                            break;
                        }
                    }
                ...
                // 기존데이터 순회하면서 엑셀데이터에 없는 것 배열에 추가
                for(const item2 of oldData) {
                    let isDuplicate = false;
                    for(const item1 of data) {
                        if(item2.optValue1 == item1.optValue1 &&
                            item2.optValue2 == item1.optValue2 &&
                            item2.optValue3 == item1.optValue3 &&
                            item2.optValue4 == item1.optValue4) {
                            isDuplicate = true;
                            break;
                        }
                    }
                    if(!isDuplicate) {
                        uniqueItems.push(item2);
                    }
                }
                ...
                for(const item of uniqueItems) {
                    if(item.tagId) {
                        $('#'+ item.tagId).find('td').eq(0).find('button').trigger('click');
                    } else {
                ...
                // 처리한 html에 엑셀데이터 바인드
                $('tr.itemoption:visible', $('#tbody_item')).each(function (idx, obj) {
                    if(data[idx].standardPriceYn == 'Y') {
                        $(this).find('td').eq(1).find('label').trigger('click');
                    }
                    $("input:text[name=attrValue1]", $(this)).val(data[idx].optValue1);
                    $("input:text[name=attrValue2]", $(this)).val(data[idx].optValue2);
                    $("input:text[name=attrValue3]", $(this)).val(data[idx].optValue3);
                    $("input:text[name=attrValue4]", $(this)).val(data[idx].optValue4);
                    $('input:text[name=cost]', $(this)).val(data[idx].cost);
                    $("input:text[name=customerPrice]", $(this)).val(data[idx].customerPrice);
                    $("input:text[name=salePrice]", $(this)).val(data[idx].salePrice);
                    $("input:text[name=supplyPrice]", $(this)).val(data[idx].supplyPrice);
                    $("input:text[name=stockQtt]", $(this)).val(data[idx].stockQtt);
                    $("input:text[name=itemErpItmCode]", $(this)).val(data[idx].erpItmCode);
                });
            ...
            function changeConFilterOptionValue(selectedFilterNo) {
                ...
                Dmall.AjaxUtil.getJSON(url, param, function (result) {
                    ...
                    var template2 =     '<label for="chk_goods_filter_{{id}}" class="chack mr20">'+
                                            '<span class="ico_comm">' +
                                                '<input type="hidden" id="${filterTypeCd}" value="{{filterNm}}" data-bind="filter_division_info" data-bind-value="goodsDavisionFilter">' +
                    ...
                    jQuery.each(result.resultList, function (idx, obj) {
                        ...
                        jQuery.each(result.resultList, function (idx2, obj2) {
                            if(obj2.parent === obj.id && obj2.filterLvl === '4') {
                                obj2.filterNm = obj.text +'_'+ obj2.text;
            ...
            // 상품 검색 데이터 팝업
            function erpItemSearchPop(target, optionType) {
                ...
                // 기본은 상품코드 검색
                $('label[for=radio_erp_itm_srch_itmCode]').trigger('click');
                // 결과 화면 숨기기
                ...
                // 입력값 초기화
                $('#layer_erp_itm_code').find('tbody').find('input[type=text], input[type=hidden]').val('');
                // 단일옵션일 때 호출된건지 다중옵션일 때 호출된건지 저장
                $('#layer_erp_itm_code').data('option-type', optionType);
            ...
            function searchErpItems() {
                ...
                if (searchType == "itmCode") {
                    if (jQuery("#erpGoodsSearchForm input[name=itmCode]").val() == "") {
                        Dmall.LayerUtil.alert("상품코드를 입력해주세요.");
                        return;
                    }
                } else if (searchType == "keyword") {
                    if (jQuery("#erpGoodsSearchForm select[name=itmKind]").val() == "") {
                        Dmall.LayerUtil.alert("상품구분을 선택해주세요.");
                        return;
                    }
                    if (jQuery("#erpGoodsSearchForm input[name=brandCode]").val() == "" && jQuery("#erpGoodsSearchForm input[name=itmName]").val() == "") {
                        Dmall.LayerUtil.alert("브랜드 혹은 상품명을 입력해주세요.");
                        return;
                    }
                } else {
            ...
            function setErpItmSearchResult(result) {
                ...
                if (prdList == null || prdList.length == 0) {
                    prdListHtml += "<tr>";
                    prdListHtml += "<td colspan='6'>데이터가 없습니다.</td>";
                    prdListHtml += "</tr>";
                    jQuery("#apply_erp_itm_code").hide();
                    ...
                    for (var i = 0; i < prdList.length; i++) {
                        ...
                        prdListHtml += "			<input type='hidden' name='frameFilterDaviBrandCode' id='frameBrandCode' value='" + prd.brandCode + "' />";
                        prdListHtml += "			<input type='hidden' name='frameFilterDaviBrand' id='브랜드' value='" + prd.brand + "' />";
                        ...
                        prdListHtml += "			<input type='hidden' name='frameFilterDaviMaterial' id='소재' value='" + prd.frameMaterial + "' />";
                        ...
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviKinds' id='종류' value='" + prd.lensKinds + "' />";
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviCorrectionCode' id='lensCorrection' value='" + prd.lensCorrectionCode + "' />";
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviCorrection' id='시력교정' value='" + prd.lensCorrection + "' />";
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviProtectionCode' id='lensProtection' value='" + prd.lensProtectionCode + "' />";
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviProtection' id='시력보호' value='" + prd.lensProtection + "' />";
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviAgeCode' id='lensAge' value='" + prd.lensAgeCode + "' />";
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviAge' id='연령대' value='" + prd.lensAge + "' />";
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviManufacturerCode' id='lensManufacturer' value='" + prd.lensManufacturerCode + "' />";
                        prdListHtml += "			<input type='hidden' name='lensFilterDaviManufacturer' id='제조사' value='" + prd.lensManufacturer + "' />";
                        ...
                        prdListHtml += "			<input type='hidden' name='contFilterDaviKinds' id='시력구분' value='" + prd.contKinds + "' />";
                        prdListHtml += "			<input type='hidden' name='contFilterDaviWearCode' id='contWearCode' value='" + prd.contWearCode + "' />";
                        prdListHtml += "			<input type='hidden' name='contFilterDaviWear' id='착용주기' value='" + prd.contWear + "' />";
                        prdListHtml += "			<input type='hidden' name='contFilterDaviColorCode' id='contColorCode' value='" + prd.contColorCode + "' />";
                        prdListHtml += "			<input type='hidden' name='contFilterDaviColor' id='컬러' value='" + prd.contColor + "' />";
                        prdListHtml += "			<input type='hidden' name='contFilterDaviDiameterCode' id='contDiameterCode' value='" + prd.contDiameterCode + "' />";
                        prdListHtml += "			<input type='hidden' name='contFilterDaviDiameter' id='그래픽직경' value='" + prd.contDiameter + "mm' />";
                        ...
                        prdListHtml += "			<input type='hidden' name='basicDaviMmft' id='mmft' value='" + prd.makName + "' />";
                        prdListHtml += "			<input type='hidden' name='basicDaviCprc' id='cost' value='" + prd.cprc + "' />";
                        prdListHtml += "			<input type='hidden' name='basicDaviCustomerPrice' id='customerPrice' value='" + prd.salePrc + "' />";
                        ...
                        if (Number(prd.sph) && Number(prd.cyl)) {
                            prdListHtml += "		<br>[SPH: " + prd.sph + " / CYL: " + prd.cyl + "]";
                        }
                        prdListHtml += "	</td>";
                        prdListHtml += "	<td>" + prd.jego + "</td>";
                        prdListHtml += "	<td>" + prd.makName + "</td>";
                        prdListHtml += "	<td>" + prd.brand + "</td>";
                        prdListHtml += "</tr>";
            ...
            function setDavisionFilterList(data, obj, bindName, target, area, row) {
                ...
                $.each(value, function (idx, filter) {
                    $('#tbody_goods_filter input[id^="' + '${filterTypeCd}' + '"]').each(function () {
                        ...
                        if(filter.toLowerCase() === $(this).val().toLowerCase()) {
                            var label = $(this).closest("label");
                            label.children('span').children('input').eq(1).prop("checked", true);
                            label.addClass('on');
                        }
                    });
                });
            ...
            function erpItemBrandSearchPop() {
                // 기존에 입력된 브랜드 정보 초기화
                $('#brandNameInput, #brandCodeInput', '#layer_erp_itm_code').val('');
                // 입력 값 초기화
            ...
            function checkValidation() {
                ...
                var dlgtImgChkLen = 0;
                var displayTypeChkLen = 0;
                //유형 체크
                $("#tr_goodsDisplay").find("input:checkbox").each(function () {
                    var $checkbox = $(this);
                ...
                if (displayTypeChkLen == 0) {
                    Dmall.LayerUtil.alert('상품 전시 유형을 설정하세요.');
                    return false;
                }
                ...
                */
                ...
                if ($('#goodsTypeCd').val() != '05' && $("#tbody_selected_ctg").find("tr.selectedCtg:visible").length < 1) {
                ...
                if ($("#hd_multiOptYn").val() == "N") {
                    ...
                    if($("#txt_customer_price").val() == null || $("#txt_customer_price").val() == '') {
                        $("#txt_customer_price").val('0');
                    }
                    ...
                    if ($("#th_multiOptErpItmCode").css("display") != "none") {
                        ...
                        $("tr.goods_item", "#tbody_goods_item").each(function () {
                            ...
                            if (data["registFlag"] != 'D') {
                                ...
                                if(data["customerPrice"] == null || data["customerPrice"] == '') {
            ...
            function getGoodsInfo(goodsNo) {
                ...
                Dmall.AjaxUtil.getJSON(url, param, function (result) {
                    ...
                    // 취득결과 셋팅
                    //
                    data['brand'] = result.data.brandNm;
            ...
            function loadBasicInfo(data) {
                ...
                */
                ...
                //     $("#lb_dcPriceApplyAlwaysYn").find("input[type=checkbox]").prop("checked", true);
                //     $("#lb_dcPriceApplyAlwaysYn").toggleClass('on');
                // }
            ...
            function getGoodsSimpleSaleValue($target, data, allDataFlag) {
                getInputDataValue(data, $("[data-bind-value=cost]", $target), $("[data-bind-value=cost]", $target).val(), allDataFlag);
                ...
                getInputDataValue(data, $("[data-bind-value=applyDavisionStockYn]", $target), $("[data-bind-value=applyDavisionStockYn]", $target).prop('checked') ? "Y" : "N", allDataFlag);
            ...
            function getGoodsMultiSaleValue($target, goodsData, allDataFlag) {
                ...
                getInputDataValue(goodsData, $("[data-bind-value=applyDavisionStockYn]", $target), $("[data-bind-value=applyDavisionStockYn]", $target).prop('checked') ? "Y" : "N", allDataFlag);
                ...
                $("tr.goods_item", "#tbody_goods_item").each(function () {
                    ...
                    var applyDavisionStockYn = $("[data-bind-value=applyDavisionStockYn]", $target).prop('checked') ? 'Y' : 'N';
                    data['applyDavisionStockYn'] = applyDavisionStockYn;
            ...
            function loadItemData() {
                ...
                $("tr.itemoption", $("#tbody_item")).each(function (idx, obj) {
                    ...
                    var salePrice = $("input:text[name=salePrice]", $(this)).val().trim().replaceAll(',', '');
                    var supplyPrice = $("input:text[name=supplyPrice]", $(this)).val().trim().replaceAll(',', '');
                    data['sepSupplyPriceYn'] = salePrice == supplyPrice ? 'N' : 'Y';
                    ...
                    $.each(imageSetList, function (idx, imgSetData) {
                        ...
                        $.each(imgDtlList, function (idx2, imgDtlData) {
                            ...
                            var data = {
                                ...
                                imgUrl: 'https://img.davichmarket.com'+ imgDtlData['imgUrl'],
            ...
            function setImageSrc(data, obj, bindName, target, area, row) {
                if (!data.type || !data.src) {
                    ...
                    if (obj.data("bind-param-2") == data.type) {
                        obj.data("img_data", data).attr("src", data.imgUrl);
                    }
                }
            }
            ...
            function getDefaultDisplayInfo(editMode) {
                ...
                Dmall.AjaxUtil.getJSON(url, param, function (result) {
                    ...
                    if (true === flag) {
                        ...
                        // 카테고리 정보 로드
                        getCategoryInfo('2', jQuery('#ul_ctg_2'), ctgNo, $('#ul_ctg_2').data('ctgNo'));
            ...
            function registDavisionStockListener() {
                // 재고 다비전 연동 체크
                $('#lb_applyDavisionStockYn').on('click', function () {
                    if($('#txt_erp_itm_code').val() && $(this).find('input').prop('checked')) {
                        var url = '/admin/goods/erp-goods-list',
                            param = { page: 1, pageNo: 0, itmCode: $('#txt_erp_itm_code').val() };

                        Dmall.AjaxUtil.getJSON(url, param, function (result) {
                            if(result.prdList == null || result.prdList.length == 0) {
                                return;
                            }

                            $('#txt_stock_qtt').val(result.prdList[0].jego);
                        });
                    }
                });

                // 물류재고 연동 (다중옵션)
                $('#lb_multiDavisionStockYn').on('click', function () {
                    if($('#tbody_goods_item').children('tr').length > 2 && $(this).find('input').prop('checked')) {
                        var itmCodes = {};
                        $('#tbody_goods_item').children('tr').each(function (idx, obj) {
                            if(idx > 1) {
                                var tdLength = $(this).children('td').length;
                                var itmCode = $(this).children('td').eq(tdLength - 1).text();
                                itmCodes[itmCode] = $(this);
                            }
                        });

                        var url = '/admin/goods/erp-goods-list',
                            param = 'page=1&pageNo=0';
                        Object.keys(itmCodes).forEach(value => {
                            param += ('&itmCodes=' + value);
                        });
                        ...
                        Dmall.AjaxUtil.getJSON(url, param, function (result) {
                            ...
                            $.each(result.prdList, function (idx, obj) {
                                var $obj = itmCodes[obj.itmCode];
                                var tdLength = $obj.children('td').length;
                                $obj.children('td').eq(tdLength - 3).text(obj.jego);
                            });
                        });
                    }
                });
            ...
            */
            ...
            function getGoodsBasicValue($target, allDataFlag) {
                ...
                getInputDataValue(returnData, $("[name=goodsNo]", $target), $("[name=goodsNo]", $target).val(), allDataFlag);
                ...
                getInputDataValue(returnData, $("[data-bind-value=brand]", $target), $("select[data-bind-value=brand] option:selected", $target).val(), allDataFlag);
            ...
            function fn_callback_pop_apply_goods(data) {
                ...
                // 검색결과 바인딩
                data['relateGoodsImg'] = data.goodsImg02;
            ...
            function setRelateImage(data, obj, bindName, target, area, row) {
                ...
                $("img", obj).attr("src", '${_IMAGE_DOMAIN}' + imgUrl);
            }
    ...
    <t:putAttribute name="content">
        <div class="sec01_box">
            ...
            <form name="form_goods_info" id="form_goods_info">
                <input type="hidden" id="goodsTypeCd" name="goodsTypeCd" value="${typeCd}"/>
                <div class="line_box fri marginB40">
                    ...
                    <div class="tblw tblmany">
                        <table id="tb_goods_basic_info"
                               summary="이표는 상품 기본정보 표 입니다. 구성은 상품번호, 상품코드, 상품명, 간단설명, 판매상태, 전시상태, 과세/비과세, 성인상품, 브랜드, 아이콘, 반품 가능 여부, 최소구매수량, 최대구매수량, 마켓포인트 설정, 수출입상품코드(HS코드) 입니다.">
                            ...
                            <tbody>
                                <tr data-bind="basic_info">
                                    ...
                                    <td id="td_goods_no">
                                        ...
                                        <input type="hidden" id="hd_goodsNo" name="goodsNo" value="${resultModel.data.goodsNo}"/>
                                        <input type="hidden" id="hd_itemNo" name="itemNo" data-bind="basic_info" data-bind-type="text" data-bind-value="itemNo"/>
                                        <input type="hidden" id="hd_brandNo" name="brandNo" data-bind="basic_info" data-bind-type="text" data-bind-value="brandNo"/>
                                ...
                                <tr data-bind="basic_info">
                                    <th>판매자 <span class="important">*</span></th>
                                    <td colspan="3">
                                    <span class="select">
                                        <label for="">판매자 선택</label>
                                        <select name="sellerNo" id="sel_seller" data-bind="basic_info"
                                                data-bind-type="labelselect" data-bind-value="sellerNo"
                                                data-validation-engine="validate[required]]">
                                            <code:sellerOption siteno="${siteNo}" includeSelect="true"/>
                                        </select>
                                    </span>
                                    </td>
                                </tr>
                                <tr data-bind="basic_info" id="tr_erp_itm_code">
                                    <th>다비젼 상품코드</th>
                                    <td colspan="3">
                                        <span class="intxt long">
                                            <input type="text" name="erpItmCode" id="txt_erp_itm_code"
                                                   data-bind="basic_info" data-bind-type="text"
                                                   data-bind-value="erpItmCode"
                                                   data-validation-engine="validate[]" readonly/>
                                        </span>
                                        <span class="btn_box">
                                            <a href="#none" id="btn_erpItm" class="btn--black_small">검색</a>
                                        </span>
                                    </td>
                                </tr>
                                <tr data-bind="basic_info">
                                ...
                                <tr data-bind="basic_info">
                                    ...
                                    <td colspan="3">
                                        <span class="select">
                                            ...
                                            <select name="brandNo" id="sel_goods_brand" data-bind="basic_info" data-bind-value="brand" data-bind-type="textselect">
                                ...
                                <tr data-bind="basic_info" id="tr_goodsDisplay">
                                    <th>유형</th>
                                ...
                                <c:if test="${typeCd eq '04'}">
                                    <tr data-bind="basic_info">
                                        <th>스탬프 사용</th>
                                        <td colspan="3" id="td_goodsStamp" data-bind="basic_info"
                                            data-bind-value="goodsStampTypeCd" data-bind-type="function"
                                            data-bind-function="setLabelRadio">
                                            <label for="rdo_stamp_type_00" class="radio mr20">
                                                <span class="ico_comm"><input type="radio" name="goodsStampTypeCd"
                                                                              id="rdo_stamp_type_00" value="00"
                                                                              data-bind="goodsStampTypeCd"/></span>
                                                적용 안함
                                            </label>
                                            <label for="rdo_stamp_type_01" class="radio mr20">
                                                <span class="ico_comm"><input type="radio" name="goodsStampTypeCd"
                                                                              id="rdo_stamp_type_01" value="01"
                                                                              data-bind="goodsStampTypeCd"/></span>
                                                10P 스탬프 상품
                                            </label>
                                            <label for="rdo_stamp_type_02" class="radio mr20">
                                                <span class="ico_comm"><input type="radio" name="goodsStampTypeCd"
                                                                              id="rdo_stamp_type_02" value="02"
                                                                              data-bind="goodsStampTypeCd"/></span>
                                                20P 스탬프 상품
                                            </label>
                                            <label for="rdo_stamp_type_03" class="radio mr20">
                                                <span class="ico_comm"><input type="radio" name="goodsStampTypeCd"
                                                                              id="rdo_stamp_type_03" value="03"
                                                                              data-bind="goodsStampTypeCd"/></span>
                                                30P 스탬프 상품
                                            </label>
                                        </td>
                                ...
                                <tr data-bind="basic_info">
                                    ...
                                    <td colspan="3" data-bind="basic_info" data-bind-value="goodsSaleStatusCd"
                                        ...
                                        <label for="rdo_goods_status_3" class="radio mr20 forceHide">
                                            <span class="ico_comm">
                                                <input type="radio" name="goodsSaleStatusCd" id="rdo_goods_status_3"
                                                       value="3" data-bind="goodsSaleStatusCd"/>
                                            </span> 판매대기
                                        </label>
                                    </td>
                                </tr>
                                ...
                                <tr data-bind="basic_info">
                                    ...
                                    <%--<tr  data-bind="basic_info" id="tr_icon_image_template">
                                        <td rowspan="3" data-bind-type="function" data-bind="basic_info" data-bind-value="idx" data-bind-function="setIconList">
                                            <label for="rdo_goodsIcon" class="radio">
                                                <span class="ico_comm">
                                                    <input type="radio" name="goodsIcon" id="rdo_goodsIcon" />
                                                </span>
                                                <img src="/admin/img/product/tmp_img01.png" width="110" height="110" alt=""
                                                     data-bind-param-1="GOODS" data-bind-param-2="02" data-bind-type="function" data-bind="basic_info" data-bind-value="idx" data-bind-function="setImageSrc"/>
                                            </label>
                                        </td>
                                    </tr>--%>
                                <tr data-bind="basic_info">
                                ...
                                <c:if test="${typeCd ne '04'}">
                                    <%--<tr data-bind="basic_info">
                                        <th>스마트피팅 사용 여부</th>
                                        <td colspan="3" data-bind="basic_info" data-bind-value="smfUseYn"
                                            data-bind-type="function" data-bind-function="setLabelRadio">
                                            <label for="rdo_smf_use_y" class="radio mr20">
                                        <span class="ico_comm">
                                            <input type="radio" name="smfUseYn" id="rdo_smf_use_y" value="Y"
                                                   data-bind="smfUseYn"/>
                                        </span> 사용
                                            </label>
                                            <label for="rdo_smf_use_n" class="radio mr20">
                                        <span class="ico_comm">
                                            <input type="radio" name="smfUseYn" id="rdo_smf_use_n" value="N"
                                                   data-bind="smfUseYn"/>
                                        </span> 미사용
                                            </label>
                                        </td>
                    ...
                    <table summary="이표는 상품관리 필터 정보 표입니다, 구성은  브랜드, 사이즈, 모양, 구조, 소재, 컬러 항목입니다."
                        ...
                        <tbody id="tbody_goods_filter">
                            ...
                            <c:if test="${typeCd eq '04'}">
                                    ...
                                    <tr data-bind="filter_davision_info" data-bind-type="function" data-bind-function="setDavisionFilterList" data-bind-value="goodsDavisionFilterList">
                                        ...
                                        <td colspan="3">
                                            <span class="select">
                                                ...
                                                <select id="sel_filter_type" name="filterNoLvl2" data-bind="filter_info" data-bind-value="filterNoLvl2" data-bind-type="labelselect">
                                            <c:forEach var="filter" items="${resultFilter}" varStatus="status">
                                                <c:if test="${filter.filterLvl eq '2' && filter.goodsTypeCd eq typeCd}">
                                                    <%--<input type="hidden"
                                                           id="${filterTypeCd}"
                                                           value="${filter.id}" data-bind="filter_division_info"
                                                           data-bind-value="goodsDavisionFilter"/>--%>
                                                    <option id="opt_filter_def_${filter.id}"
                                                            value="${filter.id}">${filter.text}</option>
                                                </c:if>
                                            </c:forEach>
                                                </select>
                                            </span>
                                ...
                                <c:forEach var="filter1" items="${resultFilter}" varStatus="status">
                                    <c:if test="${filter1.filterLvl eq '3' && filter1.goodsTypeCd eq typeCd}">
                                        <c:set var="filter_no" value="${filter1.id}"/>
                                        <c:set var="up_filter_no" value="${filter1.parent}"/>
                                        <c:set var="filter_nm" value="${filter1.text}"/>
                                    <tr id="tr_filter_menu" data-bind="filter_davision_info" class="searchFilterResult" data-bind-type="function" data-bind-function="setDavisionFilterList" data-bind-value="goodsDavisionFilterList">
                                        <th>${filter_nm}</th>
                                        <td colspan="3" data-bind="filter_info" data-bind-type="function" data-bind-function="setFilterList" data-bind-value="goodsFilterList">
                                        <c:forEach var="filter2" items="${resultFilter}" varStatus="status2">
                                            <c:set var="filter_child_id" value="${filter2.id}"/>
                                            ...
                                            <c:if test="${filter_no eq filter2.parent && filter2.filterLvl eq '4'}">
                                                <label for="chk_goods_filter_${filter_child_id}" class="chack mr20">
                                                    <span class="ico_comm">
                                                        <input type="hidden"
                                                               id="${filterTypeCd}"
                    ...
                    <div class="tblw ">
                        <table summary="이표는 상품관리 내얼굴 정보 표 입니다. 구성은 모양 컬러 사이즈 입니다.">
                            ...
                            <tbody id="tbody_goods_face">
                                ...
                                <c:if test="${typeCd eq '04'}">
                                    <tr data-bind="face_info" data-bind-type="function" data-bind-function="setFaceInfo"
                                        data-bind-value="edShape">
                                        <th>눈 모양</th>
                                        <td colspan="3" data-bind="face_info">
                                            <label for="chk_ed_shape_s" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edShape" id="chk_ed_shape_s" value="S"
                                                           data-bind="edShape"/>
                                                </span>
                                                S(Single)
                                            </label>
                                            <label for="chk_ed_shape_i" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edShape" id="chk_ed_shape_i" value="I"
                                                           data-bind="edShape"/>
                                                </span>
                                                I(In-line)
                                            </label>
                                            <label for="chk_ed_shape_o" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edShape" id="chk_ed_shape_o" value="O"
                                                           data-bind="edShape"/>
                                                </span>
                                                O(Out-line)
                                            </label>
                                    ...
                                    <tr data-bind="face_info" data-bind-type="function" data-bind-function="setFaceInfo"
                                        data-bind-value="edSize">
                                        <th>사이즈</th>
                                        <td colspan="3" data-bind="face_info">
                                            <label for="chk_ed_size_s" class="chack mr20">
                                            <span class="ico_comm">
                                                <input type="checkbox" name="edSize" id="chk_ed_size_s" value="S"
                                                       data-bind="edSize"/>
                                            </span>
                                                S(Small)
                                            </label>
                                            <label for="chk_ed_size_m" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edSize" id="chk_ed_size_m" value="M"
                                                           data-bind="edSize"/>
                                                </span>
                                                M(Medium)
                                            </label>
                                            <label for="chk_ed_size_l" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edSize" id="chk_ed_size_l" value="L"
                                                           data-bind="edSize"/>
                                                </span>
                                                L(Large)
                                            </label>
                                    ...
                                    <tr data-bind="face_info" data-bind-type="function" data-bind-function="setFaceInfo"
                                        data-bind-value="edStyle">
                                        <th>스타일</th>
                                        <td colspan="3" data-bind="face_info">
                                            <label for="chk_ed_style_c" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edStyle" id="chk_ed_style_c" value="C"
                                                           data-bind="edStyle"/>
                                                </span>
                                                C(Casual)
                                            </label>
                                            <label for="chk_ed_style_s" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edStyle" id="chk_ed_style_s" value="S"
                                                           data-bind="edStyle"/>
                                                </span>
                                                S(Street)
                                            </label>
                                            <label for="chk_ed_style_m" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edStyle" id="chk_ed_style_m" value="M"
                                                           data-bind="edStyle"/>
                                                </span>
                                                M(Minimal)
                                            </label>
                                    ...
                                    <tr data-bind="face_info" data-bind-type="function" data-bind-function="setFaceInfo"
                                        data-bind-value="edColor">
                                        <th>동공 색</th>
                                        <td colspan="3" data-bind="face_info">
                                            <label for="chk_ed_color_l" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edColor" id="chk_ed_color_l" value="L"
                                                           data-bind="edColor"/>
                                                </span>
                                                L(Light Brown)
                                            </label>
                                            <label for="chk_ed_color_b" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edColor" id="chk_ed_color_b" value="B"
                                                           data-bind="edColor"/>
                                                </span>
                                                B(Brown)
                                            </label>
                                            <label for="chk_ed_color_d" class="chack mr20">
                                                <span class="ico_comm">
                                                    <input type="checkbox" name="edColor" id="chk_ed_color_d" value="D"
                                                           data-bind="edColor"/>
                                                </span>
                                                D(Dark Brown)
                                            </label>
            ...
            <div class="line_box fri marginB40">
                        ...
                        <table summary="이표는 옵션 정보 표 입니다. 구성은 소비자가격, 공급가, 판매가격, 재고, 상품 판매기간 입니다.">
                            ...
                            <tbody id="simpleOption">
                                <tr data-bind="sale_info">
                                    <th>원가</th>
                                    <td><span class="intxt"><input type="text" class="comma" name="cost"
                                                                   id="txt_cost" data-bind="basic_info"
                                    ...
                                    <td>
                                        ...
                                        <label for="sepSupplyPriceYn" id="lb_sepSupplyPriceYn" class="chack">
                                            <input type="checkbox" id="sepSupplyPriceYn" name="sepSupplyPriceYn"
                                                   ...
                                                   data-bind-value="sepSupplyPriceYn" class="blind">
                                            <span class="ico_comm"></span>
                                            별도 공급가 적용
                                        </label>
                                    </td>
                                ...
                                <tr data-bind="sale_info">
                                    ...
                                    <td>
                                        <span class="intxt">
                                            <input type="text" class="comma" name="stockQtt"
                                        ...
                                        <label for="applyDavisionStockYn" id="lb_applyDavisionStockYn" class="chack">
                                            <input type="checkbox" id="applyDavisionStockYn" name="applyDavisionStockYn" value="Y"
                                                   data-bind="basic_info" data-bind-type="function"
                                                   data-bind-function="setLabelCheckbox" data-bind-value="applyDavisionStockYn" class="blind">
                                            <span class="ico_comm"></span>
                                            다비전 연동
                                        </label>
                                    </td>
                ...
                <div id="div_multi_option" class="change_tbl" style="display:none;">
                    <h3 class="tlth3 btn1">
                        상품 판매정보
                        <a href="#none" class="btn_gray mr20" id="btn_create_goods_option">옵션 만들기</a>
                        <label for="multiDavisionStockYn" id="lb_multiDavisionStockYn" class="chack">
                            <input type="checkbox" name="applyDavisionStockYn" id="multiDavisionStockYn" value="Y" class="blind"
                                   data-bind="basic_info" data-bind-type="function"
                                   data-bind-function="setLabelCheckbox" data-bind-value="applyDavisionStockYn">
                            <span class="ico_comm"></span>
                            물류재고 연동
                        </label>
                        ...
                        <table id="tb_goods_item"
                            ...
                            <thead>
                                <tr id="tr_goods_item_head_1">
                                    ...
                                    <th rowspan="2">재고</th>
                                    <th rowspan="2">이력</th>
                                    <th rowspan="2" id="th_multiOptErpItmCode">다비젼상품코드</th>
                                </tr>
                            ...
                            <tbody id="tbody_goods_item">
                                ...
                                <tr data-bind="sale_info" id="tr_goods_item_template" style="display:none">
                                    <td data-bind="goods_item_info" data-bind-type="function"
                                        ...
                                        <input type="hidden" name="dcStartDttm" data-bind="goods_item_info" id="hd_dcStartDttm" data-bind-type="text" data-bind-value="dcStartDttm"/>
                                        <input type="hidden" name="dcEndDttm" data-bind="goods_item_info" id="hd_dcEndDttm" data-bind-type="text" data-bind-value="dcEndDttm"/>
                                        <input type="hidden" name="dcPriceApplyAlwaysYn" data-bind="goods_item_info" id="hd_dcPriceApplyAlwaysYn" data-bind-type="text" data-bind-value="dcPriceApplyAlwaysYn"/>
                                        <label for="rdo_standardPriceYn01" class="radio mr20">
                                            <span class="ico_comm">
                                                <input type="radio" name="standardPriceYn" id="rdo_standardPriceYn01"/>
                                            </span>
                                        </label>
                                    </td>
                                    ...
                                    <td name="td_cost" data-bind="goods_item_info" data-bind-type="commanumber"
                                        data-bind-value="cost"></td>
                                    ...
                                    <td name="td_erpItmCode" data-bind="goods_item_info" data-bind-type="string"
                                        data-bind-value="erpItmCode"></td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <table summary="이표는 상품관리 사은품 정보표 입니다.">
                        ...
                        <tbody id="tbody_freebie_goods">
                            <tr>
                                <th>사은품 선택</th>
                                <td>
                                  <button type="button" id="search_freebie" class="btn--black_small mb10"> 사은품 찾기</button>
            ...
            <form name="form_goods_info2" id="form_goods_info2">
                <c:if test="${typeCd ne '04' and typeCd ne '03'}">
                <div class="line_box fri marginB40">
                                ...
                                <tr data-bind="delivery_info">
                                    ...
                                    <td>평균
                                        <span class="intxt shot">
                                            <input type="text" class="numeric" class="w50"
                                                                           name="dlvrExpectDays"
        ...
        -->
                ...
                <div class="line_box fri marginB40">
                    ...
                    <c:if test="${typeCd eq '03'}">
                        <input type="hidden" name="notifyNo" id="sel_goods_notify" value="1006">
                    </c:if>
                    <c:if test="${typeCd eq '04'}">
                        <input type="hidden" name="notifyNo" id="sel_goods_notify" value="1007">
                    </c:if>
                    <c:if test="${typeCd eq '05'}">
                        <input type="hidden" name="notifyNo" id="sel_goods_notify" value="35">
                    </c:if>
        ...
        <div class="bottom_box">
            <div class="left">
                <button class="btn--big btn--big-white"
                        onclick="location.href='/admin/goods/sales-item?typeCd=${typeCd}'">목록
                </button>
            </div>
            <div class="right">
                <button class="btn--blue-round" id="btn_confirm2">저장하기</button>
            </div>
        </div>
        <!-- //layer_popup1 -->
        <!-- content -->
                    ...
                    <div>
                        ...
                        <div class="top_lay">
                            ...
                            <div class="select_btn_right">
                                ...
                                <button class="btn_exl mr20" id="btn_upload">
                                    <span>EXCEL UPLOAD</span>
                                    <img src="/admin/img/icons/icon-excel_up.png" alt="excel icon">
                                </button>
                                <div style="display: none;">
                                    <form action="" id="form_excel_upload" method="post">
                                        <label for="input_id_excel">
                                            <input type="file" name="excel" id="input_id_excel" accept="application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
                                        </label>
                                    </form>
                                </div>
                            </div>
                                ...
                                <table id="tb_goods_option"
                                    ...
                                    <thead>
                                        <tr id="tr_pop_goods_item_head_1">
                                            ...
                                            <th rowspan="2" id="th_multiOptPopErpItmCode">다비젼상품코드<span
                                                    class="important">*</span></th>
                                        </tr>
                                    ...
                                    <tbody id="tbody_item">
                                        <tr id="tr_item_template" class="template">
                                            ...
                                            <td data-bind-type="function" data-bind="item_info"
                                                data-bind-value="standardPriceYn"
                                                data-bind-function="setStandardPriceYn">
                                            ...
                                            <td id="td_pop_dynamic_cols" class="optionTd">
                                                <span class="intxt shot2">
                                                    <input type="text" name="attrValue1" id="txt_attr1" data-bind="item_info"
                                                           data-bind-type="text" data-bind-value="attrValue1" maxlength="50"
                                                           data-validation-engine="validate[maxSize[50]]">
                                                </span>
                                            </td>
                                            <td>
                                                <span class="intxt shot2">
                                                    <input type="text" class="comma" name="cost" id="txt_cost"
                                                           data-bind="item_info" data-bind-type="textcomma"
                                                           data-bind-value="cost" maxlength="10"
                                                           data-validation-engine="validate[maxSize[10]]">
                                                </span>원
                                            </td>
                                            <td>
                                                <span class="intxt shot2">
                                                    <input type="text" class="comma" name="customerPrice" id="txt_customerPrice"
                                                           data-bind="item_info" data-bind-type="textcomma"
                                                           data-bind-value="customerPrice" maxlength="10"
                                                           data-validation-engine="validate[maxSize[10]]">
                                                </span>원
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <span class="intxt shot2">
                                                    <input type="text" class="comma" name="supplyPrice" id="txt_supplyPrice"
                                                           data-bind="item_info" data-bind-type="textcomma"
                                                           data-bind-value="supplyPrice" maxlength="10"
                                                           data-validation-engine="validate[maxSize[10]]" readonly>
                                                </span>원
                                            </td>
                                            <td>
                                                <span class="intxt shot2">
                                            ...
                                            <td>
                                                <input type="text" name="itemErpItmCode" style="width: 80px;"
                                                       data-bind="item_info" data-bind-type="text"
                                                       data-bind-value="erpItmCode"/>
                                                <button class="btn_gray" onclick="searchErpItmCode(this, event)">검색
                                                </button>
                                            </td>
                                        </tr>
                            ...
                            <table summary="이표는 옵션 생성 및 변경 표 입니다. 구성은 옵션명, 옵션 값 입니다.">
                                ...
                                <tbody id="tbody_option">
                                    <tr id="tr_option_template">
                                        ...
                                        <td><span class="intxt wid100p"><input type="text" maxlength="20" value=""></span></td>
                                        <td><span class="intxt wid100p"><input type="text" value=""></span></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
        ...
        <div id="layer_upload_item_image" class="layer_popup">
            <div class="pop_wrap size2">
                <!-- pop_tlt -->
                <div class="pop_tlt">
                    <h2 class="tlth2">상품 이미지 등록</h2>
                    <button id="btn_close_layer_upload_item_image" class="close ico_comm">닫기</button>
                </div>
                <!-- //pop_tlt -->
                <!-- pop_con -->
                <div class="pop_con">
                    <div>
                        <form action="/admin/common/file-upload" name="imageUploadForm" id="form_id_item_imageUploadForm" method="post">
                            <p class="message txtl">이미지 사이즈 설정 값을 기준으로 자동 등록됩니다</p>
                            <span class="br"></span>
                            <span class="intxt imgup1"><input id="file_route" class="upload-name" type="text"
                                                              value="이미지선택" disabled="disabled"></span>
                            <label class="filebtn" for="input_id_item_image">파일찾기</label>
                            <input class="filebox" name="file" type="file" id="input_id_item_image" accept="image/*">
                            <span class="br2"></span>
                            <span class="imgup2">
                            <input type="hidden" id="hd_item_img_param_1" name="img_param_1"/>

                        </span>
                            <div class="btn_box txtc">
                                <button class="btn_green" id="btn_regist_item_image">등록</button>
                                <button class="btn_red" id="btn_item_cancel">취소</button>
                            </div>
                        </form>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td data-bind="relate_info" data-bind-value="relateGoodsAutoExpsSortCd"
                                            ...
                                            <label for="rd_relateGoodsAutoExpsSortCd_01" class="radio left"><span
                                                    class="ico_comm"><input type="radio"
                                                                            name="relateGoodsAutoExpsSortCd" value="01"
                                                                            id="rd_relateGoodsAutoExpsSortCd_01"></span>
                                                최근 등록순(신상품순서)</label><br>
                                            <label for="rd_relateGoodsAutoExpsSortCd_02" class="radio left"><span
                                                    class="ico_comm"><input type="radio"
                                                                            name="relateGoodsAutoExpsSortCd" value="02"
                                                                            id="rd_relateGoodsAutoExpsSortCd_02"></span>
                                                판매인기순(구매금액)</label><br>
                                            <label for="rd_relateGoodsAutoExpsSortCd_03" class="radio left"><span
                                                    class="ico_comm"><input type="radio"
                                                                            name="relateGoodsAutoExpsSortCd" value="03"
                                                                            id="rd_relateGoodsAutoExpsSortCd_03"></span>
                                                판매인기순(구매갯수)</label><br>
                                            <label for="rd_relateGoodsAutoExpsSortCd_04" class="radio left"><span
                                                    class="ico_comm"><input type="radio"
                                                                            name="relateGoodsAutoExpsSortCd" value="04"
                                                                            id="rd_relateGoodsAutoExpsSortCd_04"></span>
                                                상품평 많은순</label><br>
                                            <label for="rd_relateGoodsAutoExpsSortCd_05" class="radio left"><span
                                                    class="ico_comm"><input type="radio"
                                                                            name="relateGoodsAutoExpsSortCd" value="05"
                                                                            id="rd_relateGoodsAutoExpsSortCd_05"></span>
                                                장바구니 담기 많은 순</label><br>
                                            <label for="rd_relateGoodsAutoExpsSortCd_06" class="radio left"><span
                                                    class="ico_comm"><input type="radio"
                                                                            name="relateGoodsAutoExpsSortCd" value="06"
                                                                            id="rd_relateGoodsAutoExpsSortCd_06"></span>
                                                위시리스트담기 많은순</label><br>
                                            <label for="rd_relateGoodsAutoExpsSortCd_07" class="radio left"><span
                                                    class="ico_comm"><input type="radio"
                                                                            name="relateGoodsAutoExpsSortCd" value="07"
                                                                            id="rd_relateGoodsAutoExpsSortCd_07"></span>
                                                상품조회 많은순</label>
                                        </td>
                                    </tr>
                                </tbody>
                            ...
                            <table id="tb_preview_option" summary="이표는 옵션 미리보기 표 입니다. 구성은 컬러, 사이즈 입니다.">
                                ...
                                <tbody id="tbody_preview_option">
                                    <tr id="tr_preview_option_template" style="display:none;">
                                        <th id="th_preview_option" data-bind="preview_option_info"
                                            data-bind-type="string" data-bind-value="optNm"></th>
                        ...
                        <div class="tblw tblmany2 mt0">
                            <table summary="이표는 재고 변경 이력 표 입니다. 구성은 상품번호, 상품명, 누적입고량, 누적출고량, 현재 재고량 입니다.">
                                ...
                                <tbody id="tbody_qtt_hist_head">
                                    ...
                                    <tr>
                                        <th>상품명</th>
                                        <td data-bind="qtt_hist" data-bind-type="string" data-bind="basic_info"
                                            data-bind-value="goodsNm"></td>
                                    </tr>
                                    <tr>
                                        <th>누적입고량</th>
                                        <td data-bind="qtt_hist" data-bind-type="commanumber"
                                            data-bind-value="totalPlus">0
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>누적출고량</th>
                                        <td data-bind="qtt_hist" data-bind-type="commanumber"
                                            data-bind-value="totalMinus">0
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>현재 재고량</th>
                                        <td data-bind="qtt_hist" data-bind-type="commanumber"
                                            data-bind-value="stockQtt">0
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <table summary="이표는 재고 변경 이력 리스트 표 입니다. 구성은 일자, 재고증감, 수량 입니다.">
                                ...
                                <tbody id="tbody_qtt_hist_row">
                                    <tr id="tr_qtt_hist_template" style="display:none;">
                                        ...
                                        <td data-bind="qtt_hist" data-bind-type="string"
                                            data-bind-value="chgQttCd"></td>
                        ...
                        <div class="tblw tblmany2 mt0">
                            <table summary="이표는 판매 가격 변경 이력 표 입니다. 구성은 상품번호, 상품명, 현재 판매 가격 입니다.">
                                ...
                                <tbody id="tbody_price_hist_head">
                                    ...
                                    <tr>
                                        <th>현재 판매 가격</th>
                                        <td data-bind="price_hist" data-bind-type="commanumber"
                                            data-bind-value="salePrice">0
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
