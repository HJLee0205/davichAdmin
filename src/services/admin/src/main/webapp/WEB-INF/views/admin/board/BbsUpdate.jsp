    ...
    <t:putAttribute name="script">
        ...
        var cnt= 0;
            jQuery(document).ready(function() {
                Dmall.validate.set('formBbsListUpdate');
                ...
                // 게시판 수정 함수
                jQuery('#bbsListUpdate').on('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    titleNmSet();
                    ...
                    if(Dmall.validate.isValid('formBbsListUpdate')) {
                        Dmall.EventUtil.stopAnchorAction(e);
                        Dmall.DaumEditor.setValueToTextarea(['ta_id_content1', 'ta_id_content2']);  // 에디터에서 폼으로 데이터 세팅

                        var url = '/admin/operation/board-update';
                        var param = jQuery('#formBbsListUpdate').serialize();
                ...
                var param = {siteNo:"${so.siteNo}", bbsId:"${so.bbsId}"};
                var url = '/admin/operation/board-detail';
                Dmall.AjaxUtil.getJSON(url, param, function(result) {
                    Dmall.FormUtil.jsonToForm(result.data, 'formBbsListUpdate');
                    
                    var titleNmArr = "";
                    jQuery.each(result.data.titleNmArr, function(i, obj) {
                        titleNmArr += '<li class="txt_del" id="'+obj.titleNo+'">'+obj.titleNm+
                        '<button class="btn_del btn_comm" onclick= "return delOldTitleNm('+obj.titleNo+
                        ')" ></button></li>';
                    });
                    cnt = result.data.titleNmArr.length;
                    jQuery("#viewTitleInsert").html(titleNmArr);
                    
                    Dmall.DaumEditor.setContent('ta_id_content1', result.data.topHtmlSet); // 에디터에 데이터 세팅
                    Dmall.DaumEditor.setAttachedImage('ta_id_content1', result.data.attachImages[0]); // 에디터에 첨부 이미지 데이터 세팅
                    Dmall.DaumEditor.setContent('ta_id_content2', result.data.bottomHtmlSet); // 에디터에 데이터 세팅
                    Dmall.DaumEditor.setAttachedImage('ta_id_content2', result.data.attachImages[1]); // 에디터에 첨부 이미지 데이터 세팅
                    
                    jQuery("#bbsId").val(result.data.bbsId);
                    /*jQuery("input[name = 'bbsGbCd']").toggleDesignEleReadonly();*/
                    /*jQuery("#srch_id_bbsGbCd_"+result.data.bbsGbCd).toggleDesignEleReadonly();*/
                    /*jQuery("#bbsKindCd").toggleDesignEleReadonly();*/
                    jQuery("#oldBbsAddr").html(window.location.hostname+"/front/"+result.data.bbsAddr);
                    
                    if(result.data.bbsKindCd=="1"){
                        jQuery("#titleSet").show();
                    }else{
                        jQuery("#titleSet").hide();
                    }
                    
                    if(result.data.iconSetUseYn=="Y"){
                        jQuery("#iconViewYn").show();
                        $('#iconValue').attr('rowspan', '2');
                    }else{
                        jQuery("#iconViewYn").hide();
                    ...
                    if(result.data.topHtmlYn=="Y"){
                        jQuery("#topBottomTitle").show();
                        jQuery("#topBottomView").show();
                        jQuery("#topView").show();
                    }else{
                        if(result.data.bottomHtmlYn!="Y"){
                ...
                // 사용자단 url 복사
                jQuery('#frontUrlSave').on('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();

                    prompt("Ctrl+C를 눌러 복사하세요.", jQuery("#oldBbsAddr").text());
                });
            });
            ...
            //게시판 말머리 삭제 함수
            function delOldTitleNm(titleNoVal){
                
                var url = '/admin/operation/baord-title-delete';
                var param = {titleNo:titleNoVal};
            ...
            <div class="sec01_box">
                <div class="tlt_box">
                    ...
                    <div class="btn_box right">
                        <a href="#none" id ="bbsListUpdate" class="btn blue shot">수정하기</a>
                    </div>
                </div>
                <form action="" id="formBbsListUpdate">
                <div class="line_box fri">
                   ...
                   <div class="tblw tblmany">
                       <table summary="이표는 기본 설정 표 입니다. 구성은 게시판아이디, 게시판 명, 게시판 종류, 말머리 설정, 게시판 주소 입니다.">
                           ...
                           <tbody>
                               <tr>
                                   <th>게시판아이디</th>
                                   <td id="bind_target_id_bbsIdText">
                                       <!-- <span class="intxt">
                               ...
                               <tr>
                                   ...
                                   <td>
                                       <span class="intxt">
                                            <input type="hidden" value="" id = "bbsId" name = "bbsId" />
                                            <input type="text" value="" id="bbsNm" name = "bbsNm" data-validation-engine="validate[required], maxSize[50]]" >
                                       </span>
                               ...
                               <tr>
                                   ...
                                   <td>
                                       ...
                                       <tags:radio name="bbsGbCd" codeStr="1:커뮤니티;2:고객센터;" idPrefix="srch_id_bbsGbCd" value="" />
                                   </td>
                               </tr>
                               <tr>
                                   <th>게시판 종류</th>
                                   <td>
                                       <span class="select">
                                           <label for="bbsKindCd"></label>
                                           <select id ="bbsKindCd" name = "bbsKindCd" data-validation-engine="validate[required]" >
                                               <code:option codeGrp="BBS_KIND_CD" />
                                           </select>
                                       </span>
                                       <span class="select_desc tbl_desc">
                                           *리스트형은 일반적인 형태의 게시판을 의미합니다.<br>
                                           *갤러리형은 리스트페이지에서 업로드된 이미지가 같이 보여집니다
                                       </span>
                                   </td>
                               </tr>
                               <tr id = "titleSet">
                               ...
                               <tr>
                                   <th>게시판 주소</th>
                                   <td><span id ="oldBbsAddr"></span>  <button class="btn_gray" id="frontUrlSave">복사</button></td>
                               </tr>
                               <tr>
                                   <th>게시판 사용 여부</th>
                                   <td>
                                        <tags:radio name="useYn" codeStr="Y:사용함;N:사용안함" idPrefix="srch_id_useYn" value="" /> 
                                   </td>
                               </tr>
                           </tbody>
                       </table>
                   ...
                   <div class="tblh th_l tblmany">
                       <table summary="이표는 사용여부 및 사용자 권한 설정 표 입니다. 구성은 항목 읽기(목록보기, 글내용보기), 쓰기(글쓰기, 댓글쓰기, 답글쓰기) 입니다.">
                           <caption>사용여부 및 사용자 권한 설정</caption>
                           <colgroup>
                               <col width="16%">
                               <col width="16%">
                               <col width="17%">
                       ...
                       <table summary="이표는 상세 옵션 설정 표 입니다. 구성은 작성자 표기, 아이콘 삽입 설정/조건, 비밀글 설정, 공지글 설정, 부정태그 사용방지, 게시글 스팸방지, 댓글 스팸방지 입니다.">
                           ...
                           <tbody>
                               <tr>
                                   <th>작성자 표기</th>
                                   <td>
                                       <tags:radio name="regrDispCd" codeStr="01:이름;02:아이디" idPrefix="srch_id_regrDispCd" value="" /> 
                                   </td>
                               </tr>
                               <tr>
                                   <th id = "iconValue" rowspan="2">아이콘 삽입<br>설정/조건</th>
                                   <td>
                                        <tags:radio name="iconSetUseYn" codeStr="Y:사용함;N:사용안함" idPrefix="srch_id_iconSetUseYn" value="" /> 
                                   </td>
                               </tr>
                               <tr id="iconViewYn">
                                   <td>
                                       <tags:checkbox name="iconUseYnHot" id="srch_id_iconUseYnHot" value="Y" compareValue="" text="<strong class='point_c1'>HOT</strong>" />
                                            조회수<span class="intxt shot"><input type="text" id="iconCheckValueHot" name = "iconCheckValueHot"
                                             maxlength="3" data-validation-engine="validate[custom[onlyNum]], maxSize[3]]" value = "" >
                                       </span> 회 이상 &nbsp; &nbsp; &nbsp; &nbsp;
                                       <tags:checkbox name="iconUseYnNew" id="srch_id_iconUseYnNew" value="Y" compareValue="" text="<strong class='point_c3'>NEW</strong>" />
                                       <span class="intxt shot"><input type="text" id="iconCheckValueNew" name = "iconCheckValueNew" 
                                             maxlength="3" data-validation-engine="validate[custom[onlyNum]], maxSize[3]]" value = "" >
                                       </span> 시간
                                   </td>
                               </tr>
                               <tr>
                                   <th>비밀글 설정</th>
                                   <td>
                                       <tags:radio name="sectLettSetYn" codeStr="Y:사용함;N:사용안함" idPrefix="srch_id_sectLettSetYn" value="" /> 
                                   </td>
                               </tr>
                               <tr>
                                   <th>공지글 설정</th>
                                   <td>
                                       <tags:radio name="noticeLettSetYn" codeStr="Y:사용함;N:사용안함" idPrefix="srch_id_noticeLettSetYn" value="" /> 
                                   </td>
                               </tr>
                               <tr>
                                   <th>게시글 스팸방지</th>
                                   <td>
                                        <tags:radio name="bbsSpamPrvntYn" codeStr="Y:사용함;N:사용안함" idPrefix="srch_id_bbsSpamPrvntYn" value="" /> 
                                   </td>
                               </tr>
                               <tr>
                                   <th>상단 HTML 표출 여부</th>
