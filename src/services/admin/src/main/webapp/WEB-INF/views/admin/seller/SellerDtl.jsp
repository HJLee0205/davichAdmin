...
pageContext.setAttribute("br", "<br>"); //br 태그ㅈ
%>
					...
					switch (inputId) {
						case 'input_id_files1':
							if($.inArray(ext, ['jpg','jpeg','png','gif','pdf']) == -1) {
								Dmall.LayerUtil.alert('jpg,jpeg,png,gif,pdf 파일만 업로드 할 수 있습니다.', '알림');
								$(this).val('');
								$('#fileBoxsNm1').val('');
							} else {
			...
			function fn_file_set() {
				...
				if (bizOrgFileNm != null && bizOrgFileNm != "") {
					var bizFile =
							...
							'<button class="btn_del btn_comm ml10" onclick= "return delFileNm(1)" ></button>';
				...
				if (bkCopyOrgFileNm != null && bkCopyOrgFileNm != "") {
					var bkCopyFile =
							...
							'<button class="btn_del btn_comm ml10" onclick= "return delFileNm(2)" ></button>';
				...
				if (etcOrgFileNm != null && etcOrgFileNm != "") {
					var etcFile =
							...
							'<button class="btn_del btn_comm ml10" onclick= "return delFileNm(3)" ></button>';
				...
				if (refOrgFileNm != null && refOrgFileNm != "") {
					var refFile =
							...
							'<button class="btn_del btn_comm ml10" onclick= "return delFileNm(5)" ></button>';
		...
		<script>
			$(document).ready(function() {
				...
				$('#btn_list').on('click', function() {
					Dmall.FormUtil.submit('/admin/seller/seller-list');
				});
				...
				$('#btn_save').on('click', function() {
					...
					if(Dmall.FileUpload.checkFileSize('form_id_sellerDtl')) {
						Dmall.LayerUtil.confirm('수정하시겠습니까?', function() {
							var url = '/admin/seller/seller-info-save';
				...
				$('#btn_approve').on('click', function() {
					Dmall.LayerUtil.confirm('판매자의 거래를 승인하시겠습니까?', function() {
						var url = '/admin/seller/seller-status-change';
						var param = {};
						var key;

						key = 'list[0].sellerNo';
						param[key] = $('#sellerNo').val();
						key = 'list[0].statusCd';
						param[key] = '02';
						key = 'list[0].aprvYn';
						param[key] = 'N';
						...
						Dmall.AjaxUtil.getJSON(url, param, function(result) {
							Dmall.validate.viewExceptionMessage(result, 'form_id_search').done(function () {
								Dmall.FormUtil.submit('/admin/seller/seller-list');
							});
						});
					});
				...
				// 관리자 메모 textarea 바인딩
				$('textarea[name=managerMemo]').text('${resultModel.data.managerMemo}');

				// 첫 아이디 생성 시 일부 input 비활성
				if ($('#firstId').val() == 'Y') {
					$('input').each(function() {
						var id = $(this).attr('id');

						// 제외 항목
						if(id == 'sellerId' || id == 'pw' || id == 'pwChk' ||
								id == 'id_check' || id == 'sellerNm' || id == 'managerNm' ||
								id == 'managerMobileNo' || $(this).attr('type') == 'hidden') {
							return true;
						}
					...
					$('#managerNm').attr('readonly', true);
					$('#managerMobileNo').attr('readonly', true);
				}
			...
			function checkInputValue(id) {
				var obj = $('#'+ id);
				if(obj.val() == null || obj.val() == undefined || obj.val().trim() == '' || obj.val().replaceAll(',', '') == '0') {
					obj.val('0');
					if(id == 'sellerSvmnAmt') {
			...
			function checkValidation() {
				...
				// 숫자 검증
				var exp = /[0-9]+\.[0-9]+/;
				if(exp.test($('#sellerCmsRate').val())) {
					Dmall.LayerUtil.alert('정수,소수의 숫자만 입력할 수 있습니다.<br/>입력필드 : 판매자 수수료율', '알림');
					return false;
				}
				if(exp.test($('#sellerSvmnAmt').val())) {
					Dmall.LayerUtil.alert('정수,소수의 숫자만 입력할 수 있습니다.<br/>입력필드 : 포인트 지급 설정', '알림');
					return false;
				}
				if(exp.test($('#sellerSvmnMaxUseRate').val())) {
					Dmall.LayerUtil.alert('정수,소수의 숫자만 입력할 수 있습니다.<br/>입력필드 : 포인트 사용 제한 설정', '알림');
					return false;
				}
				if(exp.test($('#svmnLoadrate').val())) {
					Dmall.LayerUtil.alert('정수,소수의 숫자만 입력할 수 있습니다.<br/>입력필드 : 포인트 사용 본사 부담율', '알림');
					return false;
				}
	...
	<t:putAttribute name="content">
		<div class="sec01_box">
			...
			<form id="form_id_sellerDtl" method="post">
				...
				<c:choose>
					<c:when test="${sellerDtl.sellerId == '' || sellerDtl.sellerId == null}">
						<input type="hidden" name="firstId" id="firstId" value="Y"/>
						<input type="hidden" name="chkSellerId" id="chkSellerId" value="false">
					</c:when>
					<c:otherwise>
						<input type="hidden" name="firstId" id="firstId" value="N"/>
					...
					<div class="tblw tblmany">
						<table summary="이표는 기본정보 표 입니다. 구성은 이름/아이디, 실명확인, 성별, 생년월일, 이메일/수신여부, 핸드폰/수신여부, 전화번호, 주소 입니다.">
							...
							<tr>
								...
								<td colspan="3">
									<span class="intxt"><input type="text" name="sellerId" id="sellerId" value="${sellerDtl.sellerId}" data-validation-engine="validate[required, maxSize[20]]"></span>
									<button type="button" class="btn_gray2 popup_open" id="btn_id_check">중복확인</button>
							...
							<tr id="pwTr">
								...
								<td colspan="3">
									...
									<span class="point_c6 member_password">
										* 영문/숫자/특수문자 2가지 이상 조합 8~16자로 입력하세요. 아이디가 포함되거나, 3자리 이상 반복되는 문구와 숫자는 불가합니다.
									</span>
								</td>
							</tr>
							<tr>
								...
								<td>
									<span class="intxt w100p"><input type="text" name="sellerNm" id="sellerNm" value="${sellerDtl.sellerNm}" data-validation-engine="validate[required, maxSize[30]]" maxlength="30"></span>
								</td>
								<th>사업자등록번호 <span class="important">*</span></th>
								<td>
									<span class="intxt w100p"><input type="text" name="bizRegNo" id="bizRegNo" maxlength="10" value="${sellerDtl.bizRegNo}" data-validation-engine="validate[required, maxSize[10]]"></span>
								</td>
							</tr>
							<tr>
								<th>판매자 소개</th>
							...
							<tr>
								<th>판매자 수수료율</th>
								<td colspan="3">
									<span class="intxt"><input type="text" name="sellerCmsRate" id="sellerCmsRate" value="${sellerDtl.sellerCmsRate}" ></span>%
								</td>
							</tr>
							<tr>
								<th>포인트 지급 설정</th>
								<td colspan="3">
									...
									<label for="" class="intxt shot"><input type="text" name="sellerSvmnAmt" id="sellerSvmnAmt" value="${sellerDtl.sellerSvmnAmt}" class="comma" maxlength="10"></label>
							...
							<tr>
								...
								<td colspan="3">
									...
									<label for="sellerSvmnMaxUseRate" class="intxt shot">
										<input type="text" id="sellerSvmnMaxUseRate" name="sellerSvmnMaxUseRate" maxlength="4" value="${sellerDtl.sellerSvmnMaxUseRate}">
									</label> % 을(를) 까지 사용 가능
								<tr>
								<th>포인트 사용 본사부담율</th>
								<td colspan="3">
									<label for="svmnLoadrate" class="mr5">
										<input type="text" id="svmnLoadrate" name="svmnLoadrate" maxlength="4" value="${sellerDtl.svmnLoadrate}">
									</label> %
								</td>
							</tr>
							<tr>
								...
								<th>결제계좌번호</th>
								<td>
									<span class="intxt w100p"><input type="text" name="paymentActNo" id="paymentActNo" value="${sellerDtl.paymentActNo}"></span>
								</td>
							</tr>
							<tr>
								<th>예금주명</th>
								<td>
									<span class="intxt w100p"><input type="text" name="paymentActNm" id="paymentActNm" value="${sellerDtl.paymentActNm}"></span>
								</td>
								<th>업태 <span class="important">*</span></th>
								<td>
									<span class="intxt w100p"><input type="text" name="bsnsCdts" id="bsnsCdts" value="${sellerDtl.bsnsCdts}" data-validation-engine="validate[required, maxSize[15]]"></span>
								</td>
							</tr>
							<tr>
								<th>종목 <span class="important">*</span></th>
								<td>
									<span class="intxt w100p"><input type="text" name="st" id="st" value="${sellerDtl.st}" data-validation-engine="validate[required, maxSize[15]]"></span>
								</td>
								<th>대표자명</th>
								<td>
									<span class="intxt w100p"><input type="text" name="ceoNm" id="ceoNm" value="${sellerDtl.ceoNm}"></span>
								</td>
							</tr>
							<tr>
								<th>대표 전화번호</th>
								<td>
									<span class="intxt w100p"><input type="text" name="dlgtTel" id="dlgtTel" value="${sellerDtl.dlgtTel}" onblur="chk_tel(this.value,this);" data-validation-engine="validate[maxSize[11]]"></span>
								</td>
								<th>대표 휴대폰번호</th>
								<td>
									<span class="intxt w100p"><input type="text" name="mobileNo" id="mobileNo" value="${sellerDtl.mobileNo}" onblur="chk_tel(this.value,this);" data-validation-engine="validate[maxSize[11]]"></span>
								</td>
							</tr>
							<tr>
								<th>팩스</th>
								<td>
									<span class="intxt w100p"><input type="text" id="fax" name="fax" value="${sellerDtl.fax}" onblur="chk_tel(this.value,this);" datetimeonly="true" style="ime-mode:disabled;" data-validation-engine="validate[maxSize[11]]"></span>
								</td>
							...
							<tr>
								...
								<th>사업자 등본</th>
								<td>
									<span class="intxt "><input class="upload-name" id="fileBoxsNm1" type="text" value="" disabled></span>
							...
							<tr>
								<th>통장 사본</th>
								<td>
									<span class="intxt wid240"><input class="upload-name" id="fileBoxsNm2" type="text" value="" disabled></span>
								...
								<th>통신판매업신고증</th>
								<td>
									<span class="intxt"><input class="upload-name" id="fileBoxsNm3" type="text" value="" disabled></span>
							...
							<tr>
								<th>거래 상태</th>
								<td colspan="3">
									<tags:radio codeStr="02:거래중;03:거래정지;" name="statusCd" idPrefix="check_id_status" value="${sellerDtl.statusCd}"/>
								</td>
							</tr>
							</tbody>
						</table>
				...
				<div class="line_box fri marginB40">
					...
					<div class="txt_area">
						<c:set var="memo" value="${fn:replace(sellerDtl.managerMemo, br, cn)}"/>
						<textarea  name="managerMemo"  data-validation-engine="validate[maxSize[200]]">${memo}</textarea>
					</div>
				</div>
				<!-- //line_box 판매자 메모-->
				<!-- line_box 처리 로그 -->
				<div class="line_box fri pb">
					...
					<div class="disposal_log">
						<ul>
							<c:forEach var="log" items="${sellerDtl.chgLog}">
								<c:forEach var="logNm" items="${log.chgNm.split('/')}">
									<li>${log.chgDttm.substring(0, 19)}&nbsp;${logNm} 변경</li>
								</c:forEach>
							</c:forEach>
						</ul>
					</div>
		...
		<div class="bottom_box">
			...
			<div class="right">
				<c:choose>
					<c:when test="${sellerSO.inputGbn == 'UPDATE'}">
						<c:if test="${sellerDtl.statusCd == '01'}">
							<button class="btn--blue-round" id="btn_approve">승인 완료</button>
						</c:if>
						<button class="btn--blue-round" id="btn_save">수정</button>
					</c:when>
					<c:otherwise>
		...
		<div id="popup_id_duplicate_check" class="layer_popup">
			<div class="pop_wrap size2">
				<div class="pop_tlt">
					<h2 class="tlth2">아이디 중복확인 <span class="desc"></span></h2>
