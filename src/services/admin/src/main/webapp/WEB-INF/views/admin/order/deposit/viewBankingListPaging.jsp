...
<t:insertDefinition name="defaultLayout">
    <t:putAttribute name="title">홈 &gt; 주문 &gt; 무통장 입금관리</t:putAttribute>
    <t:putAttribute name="script">
        ...
        var getList = function() {
            var url = '/admin/order/deposit/banking-list',
            ...
            Dmall.AjaxUtil.getJSON(url, param, function(result) {
                ...
                var template4 = '<td><a href="javascript:confirmDeposit({{ordNo}})"  class="btn_gray2">결제확인</a></td>' +
                '</tr>' ;
                ...
                jQuery.each(result.resultList, function(idx, obj) {
                    ...
                    if(obj.ordStatusCd == '10') // 주문 완료인 경우
                ...
                if(tr == '') {
                    tr = '<tr><td colspan="9"><spring:message code="biz.exception.common.nodata"/></td></tr>';
                }
        ...
        // 주문완료건을 결제완료 상태로 변경
        function confirmDeposit(ordNo) {
            Dmall.LayerUtil.confirm('선택된 주문을 결제완료 하시겠습니까?', function() {
                var url = "/admin/order/manage/order-status-update";
                param = {ordNo:ordNo, ordStatusCd:'20', curOrdStatusCd:'10'};
                Dmall.AjaxUtil.getJSON(url, param, function(result) {
                    if(result.success) {
                        $(getList);
                    }else {
                        Dmall.LayerUtil.alert('결제 확인에 실패했습니다. 관리자에게 문의 바랍니다.');
                    }
                });
            });
        }
    ...
    <div class="sec01_box">
        ...
        <div class="search_box">
                <form:form id="form_ord_search" commandName="depositSO">
                    ...
                    <input type="hidden" name="paymentWayCd" id="paymentWayCd"  value="11"/>

            <!-- search_tbl -->
            <div class="search_tbl">
                <table summary="이표는 무통장 입금 관리 검색 표 입니다. 구성은 주문일, 주문상태, 판매채널, 검색어 입니다.">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th>주문상태</th>
                            <td>
                                <label for="ordStatusCd1" class="chack mr20" onclick="chack_btn(this);">
                                    <span class="ico_comm">
                                    <input type="checkbox" name="ordStatusCd" id=ordStatusCd1 class="blind"  value='10'/>
                                    </span>
                                    주문완료
                                </label>
                                <label for="ordStatusCd2" class="chack mr20" onclick="chack_btn(this);">
                                    <span class="ico_comm">
                                    <input type="checkbox" name="ordStatusCd" id="ordStatusCd2" class="blind"  value='20'/>
                                    </span>
                                    결제완료
                                </label>
