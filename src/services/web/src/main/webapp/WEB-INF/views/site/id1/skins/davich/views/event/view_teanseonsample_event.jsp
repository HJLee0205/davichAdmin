				...
				$('#btn_non_visit').on('click',function(){
					...
					if(loginYn != 'true') {
						...
						function() {
							...
							$('#visitForm').find('#returnUrl').val(HTTPS_SERVER_URL+'/front/event/teanseon-event');
					...
					Dmall.AjaxUtil.getJSON(url, param, function(result) {
						if(result.success){
							...
							if(data.shareCnt > 0){
								$("#snsShareYn").val('Y');
								$('#visitForm').attr('action','/front/goods/goods-detail?goodsNo=G2103261158_8727');
								$('#visitForm').attr('method','post');
								$('#visitForm').submit();
							}else{
    ...
    <t:putAttribute name="content">
		...
		<br>
			...
			<div class="cont_body event_tensean_area">
				<img src="${_SKIN_IMG_PATH}/event/event_210412_ts01.jpg" alt="다비치 렌즈 샘플링 신청하기 2탄">
