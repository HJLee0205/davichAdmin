...
<t:insertDefinition name="davichLayout">
		...
		<script>
			...
			jQuery(document).ready(function() {
				jQuery('#btn_id_pwchange').on('click', function () {
					if(orderYn=='Y'){
						$('#orderForm').attr("action",HTTPS_SERVER_URL + '/front/login/change-password-step2');
						$('#orderForm').submit();
					}else{
						if(visionYn=='Y'){
							$('#form_vision_check').attr("action",HTTPS_SERVER_URL + '/front/login/change-password-step2');
							$('#form_vision_check').submit();
						}else{
							$('#returnForm').attr("action",HTTPS_SERVER_URL + '/front/login/change-password-step2');
							$('#returnForm').submit();
						}
					}
					//window.location.href = HTTPS_SERVER_URL + '/front/login/change-password-step2';
				});


				jQuery('#btn_id_next').on('click', function () {
					var url ='/front/login/password-nextchange-update';
					var param = {};
	...
	<t:putAttribute name="content">
		...
		<div class="password_change_box">
			<!--step1-->
			<div id="div_id_stap1">
				<h2 class="sub_title">비밀번호 변경안내</h2>
				<div class="password_change_text01">
					고객님님의 소중한 정보 보호를 위해 <em>비밀번호를 변경</em>해 주세요.
				</div>
				<div class="password_change_text02">
					개인정보 유출로 인한 피해 사례를 막기 위해<br>
                    ...
                    <p>* 다음에 변경하기를 눌러 변경을 연기하시면 <span class="point_red">${siteInfo.pwChgNextChgDcnt}</span>일 후 다시 안내해드립니다.</p>
				</div>
				<div class="btn_change_area">
					<button type="button" class="btn_change_now" id="btn_id_pwchange">지금변경하기</button>
					<button type="button" class="btn_change_next" id="btn_id_next">다음에 변경하기</button>
				</div>
			</div>
			<!--//step1-->
		</div>
