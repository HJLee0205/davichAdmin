...
<t:insertDefinition name="davichLayout">
	<t:putAttribute name="title">다비치마켓 :: 회원가입</t:putAttribute>
		...
		<script>
			$(document).ready(function(){
				...
				/*$(".btn_member_check").click(function(){
					if(InputCheck()){
						var url = '/front/member/send-email';
						param = {
						...
						Dmall.AjaxUtil.getJSON(url, param, function(result) {
							if(result.success) {
								$(".member_warning").hide();
								$(".member_alert").show();
								$('html, body').animate({ scrollTop: $('.member_alert').offset().top }, 'slow');
								/!*Dmall.LayerUtil.alert("인증번호 발송 성공 하였습니다.", "알림");*!/

							}else{
								$(".member_alert").hide();
								$(".member_warning").show();
								$('html, body').animate({ scrollTop: $('.member_warning').offset().top }, 'slow');
	...
	<t:putAttribute name="content">
		<div id="member_container">
			<h3 class="member_sub_tit">다비치마켓에 오신 것을 환영합니다.</h3>
			<div class="member_info">
				<p>
					<span class="tit">다비치마켓(온라인쇼핑몰)과 다비치안경원과의 통합회원 가입으로<br>
					전국 250여개의 다비치 안경원에 근무하는 전문 안경사 및 청력사를 통한<br>
					안경, 안경렌즈, 콘텍트렌즈, 보청기의 차별화된 최상의 서비스를 
					경험하실 수 있습니다.</span>
					<dl class="list_type1">
						<dt>서비스</dt>
						<dd>안경테, 안경렌즈, 콘텍트렌즈, 보청기 컨설팅</dd>
						<dd>방문예약 서비스</dd>
						<dd>쇼핑몰 구매상품 <div href="#" class="tooltip">DDS(다비치 다이렉트 서비스<span class="tooltiptext">DDS(다비치 다이렉트 서비스)<br>온라인에서 주문하고 가까운 다비치 매장에서 수령<br>다비치안경의 전문적인 피팅 서비스<br>다비치안경 AI GO VCS를 통한 차별화된 비전 컨설팅 시스템</span></div>, 매장 픽업) 서비스</dd>
						<dd>포인트 결제(온라인, 오프라인) 서비스</dd>
						<dd>추가할인 서비스</dd>
						<dd>각종 행사 안내 서비스</dd>
					</dl>
				</p>
			...
			<%--<div class="member_info_bottom">
				다비치마켓은 <em>이메일 계정</em>을 회원 아이디로 사용합니다.<br>
				보다 편리하고 안정된 서비스 이용을 위해 <em>반드시 사용중인 이메일 계정으로 가입</em>해주세요.
			</div>--%>
			...
			<!-- tab01 개인회원 --->
			<div class="tabs_member_content" id="member01">
