...
<t:insertDefinition name="davichLayout">
    <t:putAttribute name="title">다비치마켓 :: 메세지함(마켓)</t:putAttribute>
    ...
    <t:putAttribute name="script">
        ...
        $(document).ready(function(){
            ...
            $('.btn_form').on('click', function() {
                ...
                Dmall.FormUtil.submit('/front/member/push-message-market', param);
            });
        });
							...
							<c:choose>
                                <c:when test="${fn:length(resultListModel.resultList) > 0}">
                                	<c:forEach var="li" items="${resultListModel.resultList }">
                               			<tr class="title">
                               				<td>${li.appDate }</td>
                                    		<td class="textL">${fn:substring(li.sendMsg,0,30)}</td>
                                    		<td>${li.appTime }</td>
                                    		<%--<td>다비치마켓</td>--%>
                                    	</tr>
                                    	<tr class="hide">
											<td colspan="3" class="my_qna_view">
												${li.sendMsg}
                                ...
                                <c:otherwise>
                                    <tr>
                                        <td colspan="3">수신받은 메세지가 없습니다.</td>
                                    </tr>
                                </c:otherwise>
                            </c:choose>
						</tbody>
