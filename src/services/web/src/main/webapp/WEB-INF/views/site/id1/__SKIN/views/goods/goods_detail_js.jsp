...
function mobRfShop(){
  ...
  document.getElementById("btn_cart_go").onmouseup = sendCart;
  function sendCart() {
    sh.sendCart();
  }
...
<script>
    ...
    $(document).ready(function(){
        ...
        ajaxNotifyList();
        //상품 배송/반품/환불 정보 호출
        if("${goodsInfo.data.goodsTypeCd}" != '01' && "${goodsInfo.data.goodsTypeCd}" != '02'){
        	ajaxGoodsExtraInfo();
        }
        //최근본상품에 담기
        setLatelyGoods();
        ...
        $('.btn_share').show();
     	...
     	if(opt == 'inquiry'){
     		$('#question_tab').click();
     		$('html, body').animate({ scrollTop: $('#question_tab').offset().top }, 'slow');
     	}else if(opt == 'review'){
     		$('#review_tab').click();
     		$('html, body').animate({ scrollTop: $('#review_tab').offset().top }, 'slow');
     	}
        ...
        $('[id^=dlvrMethodCd]').on('change',function(){
            if($(this).val() == '02') {
                $('[id^=dlvrPaymentKindCd01]').hide();
                $('[id^=dlvrPaymentKindCd02]').show();
            } else {
                $('[id^=dlvrPaymentKindCd01]').show();
                $('[id^=dlvrPaymentKindCd02]').hide();
            }
        })
        ...
        $('select.select_options').on('change',function(){
            ...
            if (optAdd) {
                ...
                if (itemInfo != '') {
                    ...
                    var itemNo = "";    //단품번호
                    var itemNm = "";    //단품명
                    var customerPrice = ${goodsInfo.data.customerPrice}
                    ...
                    if(addLayer) {
                        ...
                        optObj += '     <input type="hidden" name="itemSumCustomerPriceArr" class="itemSumCustomerPriceArr" value="'+customerPrice+'">';
                        optObj += '    </div>';
                        ...
                        if(stockQtt <= 0) {
                            ...
                            optObj += '            <span class="itemSumPriceText" ';
                            if(preGoodsYn == "Y") {
            					optObj += 'style="display:none;"';
            				}
                            optObj += '></span>';
        ...
        $('select.select_option.goods_addOption').on('change',function(){
            ...
            if($(this).find(':selected').val() != '') {
                ...
                if(addLayer) {
                    ...
                    optObj += '        </div>';
                    optObj += '        <div class="goods_price_select">';
                    optObj += '            <span class="addOptSumAmtText" '; 
                    if(preGoodsYn == "Y") {
    					optObj += 'style="display:none;"';
    				}
                    optObj += '></span>';
    ...
    function rsv_go(target_id) {
        ...
        if(formCheck) {
            ...
            /*$('#goods_form').attr('action',HTTPS_SERVER_URL+'/front/visit/visit-book');
            $('#goods_form').attr('method','post');
            $('#goods_form').submit();*/
    ...
    // 상품이미지 크게보기
    function goods_big_preview(goodsNo){
        ...
        Dmall.AjaxUtil.getJSON(url, param, function(result) {
            if(result.success) {
                if(result.resultList.length > 0) {
                    ...
                    couponLayerPop += '        <div class="popup_coupon_top">';
                    couponLayerPop += '            <button type="button" class="btn_coupon_download" onclick="issueCouponAll();">전체 다운로드</button>';
                    couponLayerPop += '        </div>';
        ...
        Dmall.AjaxUtil.getJSON(url, param, function(result) {
            if(result.success) {
                ...
                if(offYn == 'Y'){
                	...
                	//Dmall.LayerUtil.alert('쿠폰이 발급 되었습니다.<br>다운로드 받으신 쿠폰은 마이페이지에서<br>확인 가능합니다. <a href="/front/coupon/coupon-list" style="text-decoration:underline;color:blue;">마이페이지</a>', '','');
                	Dmall.LayerUtil.confirm("쿠폰이 발급 되었습니다.<br>다운로드 받으신 쿠폰은 마이페이지에서<br>확인 가능합니다.",
                        //확인버튼 클릭, 확인시 로그인페이로 이동하는등의 동작이 필요
                        function() {
    ...
    function jsSetTotalPrice() {
        var totalPrice = 0;
        var discount = $('.discount_ratio').text();
        discount = discount? (100 - Number(discount)) / 100 : 1;
        ...
        $('#totalCustomerPriceText').html(commaNumber(Number.parseInt(totalPrice / discount)));
    ...
    function jsFormValidation() {
        ...
        if(multiOptYn == 'Y' && optLayerCnt == 0) {
            $('select.select_options').each(function(){
                if($(this).find(':selected').val() == ''){
