...
<%@ page import="java.security.MessageDigest, dmall.framework.common.util.* " %>
...
<!-- script language=javascript src="http://www.allthegate.com/plugin/AGSWallet_utf8.js"></script --> 
<!-- ActiveX 방식을 삭제하고 아래 웹표준 방식으로 js URL 변경처리 -->
<script type="text/javascript" src="//www.allthegate.com/plugin/AGSWallet_New.js" charset="utf-8"></script>
...
function AlltheGatePay(form){
    // alert('PG 테스트아이디는 나이스페이먼츠 전용테스트 아이디므로 \n\n※※사용 후 바로 취소 해주시기 바랍니다.');
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(form.Flag.value == "enable"){
        ...
        if(Check_Common(form) == true){
            ...
            if(document.AGSPay == null || document.AGSPay.object == null){
                Dmall.LayerUtil.alert('AlltheGate 결제 플러그인 설치 후 다시 시도 하십시오.')
                .done(function(){console.log("AlltheGate 플러그인 설치 안됨");});// 결제인증창 호출
                ...
                //     var openwin = window.open("./04/progress","popup","width=300,height=160"); //"지불처리중"이라는 팝업창연결 부분
...
function Check_Common(form){
    if(form.StoreId.value == ""){
        Dmall.LayerUtil.alert("상점아이디를 입력하십시오.");
        return false;
    }
    else if(form.StoreNm.value == ""){
        Dmall.LayerUtil.alert("상점명을 입력하십시오.");
        return false;
    }
    else if(form.OrdNo.value == ""){
        Dmall.LayerUtil.alert("주문번호를 입력하십시오.");
        return false;
    }
    else if(form.ProdNm.value == ""){
        Dmall.LayerUtil.alert("상품명을 입력하십시오.");
        return false;
    }
    else if(form.Amt.value == ""){
        Dmall.LayerUtil.alert("금액을 입력하십시오.");
        return false;
    }
    else if(form.MallUrl.value == ""){
        Dmall.LayerUtil.alert("상점URL을 입력하십시오.");
        return false;
    }
    return true;
}
...
function Display(form){
    if(form.Job.value == "onlycard" || form.TempJob.value == "onlycard"){
        document.getElementById('card_hp').style.display= "";
        ...
        document.getElementById('card_hp').style.display= "none";
        document.getElementById('card').style.display= "";
        document.getElementById('hp').style.display= "none";
...
<!-- <form name=frmAGS_pay id=frmAGS_pay method=post action=order-insert> -->
<div id="AGS_PAYREQFORM" style="display:none;">
            ...
            <tr>
                ...
                <table width=650 border=0 cellpadding=0 cellspacing=0>
                    ...
                    <tr>
                        ...
                        <td colspan=2><input type=text style=width:100px name=StoreId maxlength=20 value="${pgPaymentConfig.data.pgId}"></td>
                    </tr>
                    <tr>
                        <td class=clsleft>☞ 주문번호 (40)</td>
                        <td colspan=2><input type=text style=width:100px name=OrdNo maxlength=40 value="${ordNo}"></td>
                    </tr>
                    <tr>
                        <td class=clsleft>☞ 금액 (12)</td>
                        <td><input type=text style=width:100px name=Amt maxlength=12 value="">원</td>
                        <td class=clsleft>예) 금액 콤마(,)입력불가</td>
                    </tr>
                    <tr>
                        <td class=clsleft>☞ 상점명 (50)</td>
                        <td colspan=2><input type=text style=width:300px name=StoreNm value="${site_info.siteNm}"></td>
                    </tr>
                    <tr>
                        <td class=clsleft>☞ 상품명 (300)</td>
                        <td colspan=2><input type=text style=width:300px name=ProdNm maxlength=300 value="상품명"></td>
                    </tr>
                    <tr>
                        <td class=clsleft>☞ 상점URL (50)</td>
                        <!-- 주의) 상점홈페이지주소를 반드시 입력해 주십시오. -->
                        <!-- (미입력시 특정 카드사 신용카드 결제 및 가상계좌 결제가 이뤄지지 않을 수 있습니다.) -->
                        <td><input type=text style=width:300px name=MallUrl value="${_DMALL_HTTPX_SERVER_URL}"></td>
                        <td class=clsleft>예) http://www.abc.com</td>
                    </tr>
                    <tr>
                        <td class=clsleft>주문자이메일 (50)</td>
                        <td colspan=2><input type=text style=width:300px name=UserEmail maxlength=50 value=""></td>
                    </tr>
                    <tr>
                        ...
                        <td colspan=2><input type=text style=width:400px name=ags_logoimg_url maxlength=200 value="${_DMALL_HTTPX_SERVER_URL}${site_info.logoPath}"></td>
                    </tr>
                    <tr>
...
<!--                         <td><input type=text style=width:300px name=SubjectData value="업체명;판매상품;계산금액;2012.09.01 ~ 2012.09.30;"></td> -->
                        <td><input type=text style=width:300px name=SubjectData value=""></td>
                ...
                <table width=650 border=0 cellpadding=0 cellspacing=0>
                    <tr>
                        ...
                        <td colspan=2><input type=text style=width:100px name=UserId maxlength=20 value="${user.session.loginId}"></td>
                    </tr>
                </table>
                </div>
                ...
                <table width=650 border=0 cellpadding=0 cellspacing=0>
                    ...
                    <tr>
                        ...
                        <td width=300><input type=text style=width:100px name=OrdNm maxlength=40 value=""></td>
                        <td width=190></td>
                    </tr>
                    <tr>
                        <td class=clsleft>주문자연락처 (21)</td>
                        <td colspan=2><input type=text style=width:100px name=OrdPhone maxlength=21 value=""></td>
                    </tr>
                    <tr>
                        <td class=clsleft>주문자주소 (100)</td><!-- 가상계좌추가 -->
                        <td colspan=2><input type=text style=width:300px name=OrdAddr maxlength=100 value=""></td>
                    </tr>
                    <tr>
                        <td class=clsleft>수신자명 (40)</td>
                        <td colspan=2><input type=text style=width:100px name=RcpNm maxlength=40 value=""></td>
                    </tr>
                    <tr>
                        <td class=clsleft>수신자연락처 (21)</td>
                        <td colspan=2><input type=text style=width:100px name=RcpPhone maxlength=21 value=""></td>
                    </tr>
                    <tr>
                        <td class=clsleft>배송지주소 (100)</td>
                        <td colspan=2><input type=text style=width:300px name=DlvAddr maxlength=100 value=""></td>
                    </tr>
                    <tr>
                ...
                <table width=650 border=0 cellpadding=0 cellspacing=0>
                    ...
                    <tr>
                        ...
                        <td width=300><input type=text style=width:300px name=MallPage value="/admin/interfaces/04/deposit-notice"></td>
...
<input type="text" name="AGS_HASHDATA" value="">     <!-- 전역 해쉬 변수 -->
