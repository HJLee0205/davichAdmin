...
<t:insertDefinition name="davichLayout">
	...
	<script type="text/javascript">
	 ...
	 $(document).on('click','.btn_go_recomm',function(){
	...
	$(".btn_result_save").click(function(){
        ...
        if(!loginYn) {
            Dmall.LayerUtil.confirm("로그인이 필요한 서비스입니다.<br>지금 로그인 하시겠습니까?",
                ...
                function() {
                    //var returnUrl = window.location.pathname+window.location.search;
                    var returnUrl = "/front/hearingaid/hearingaid-check";                                
                    location.href= "/front/login/member-login?returnUrl="+returnUrl;
                }
            );
            return;
    	...
    	Dmall.LayerUtil.confirm("결과를 저장하시겠습니까?", function(){                		
    		...
    		$.ajax({
    	 		type : "POST",
    	 		url : "/front/hearingaid/hearingaid-check-insert",
    	 		data : {
    	 			goodsNos	: v_goods_no,
    	 			goodsNms	: v_goods_nm,
    	 			ctgNos 		: v_stepCd,
    	 		...
    	 		complete: function(){				
    	 			document.location.href='/front/hearingaid/my-hearingaid-check';
    	 		}
    	 	});	
		...
		<div class="lens_recomm result">
			<div class="vision_result">
				...
				<p class="recomm_text">이명 유무
					<c:choose>
						<c:when test="${hearingaidVO.ringing == '01'}"><em>[없음]</em></c:when>
						<c:when test="${hearingaidVO.ringing == '02'}"><em>[있음]</em></c:when>
						<c:otherwise><em>[없음]</em></c:otherwise>
					</c:choose>	
				 </p>
				<p class="recomm_text">선호타입  
					<c:choose>
						<c:when test="${hearingaidVO.haType == '01'}"><em>[귓속 타입]</em></c:when>
						<c:when test="${hearingaidVO.haType == '02'}"><em>[귀걸이 타입]</em></c:when>
						<c:otherwise><em>[상관없음]</em></c:otherwise>
					</c:choose>	을 선택하신 <b>${userName}</b> 고객님에게</p>
				<p class="recomm_text">추천 드리는 보청기 입니다.</p>
			</div>
			<button type="button" class="btn_recom_again" onclick="document.location.href='/front/hearingaid/hearingaid-check?haUse=${hearingaidVO.haUse}'">보청기 추천 다시받기<i></i></button>
		</div>
