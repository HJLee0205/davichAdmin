...
<t:insertDefinition name="davichLayout">
    <t:putAttribute name="title">다비치마켓 :: 주문완료</t:putAttribute>
    ...
    <t:putAttribute name="content">
        ...
        <c:set var="lensType" value="C"/>
         ...
         <%--// logCorpAScript --%>
        <c:forEach var="goodsList" items="${orderVO.orderGoodsVO}" varStatus="status">
            <%--주문상풍유형이 [안경테, 안경렌즈, 콘택트렌즈, 선글라스, 보청기]일경우 주문완료 폼 설정--%>
        ...
        <div class="order_middle">
            <div class="order_head">
                <h2 class="order_tit">주문완료</h2>
                <ul class="order_steps">
                    <li><span class="step01"><i></i>장바구니</span></li>
                    <li><span class="step02"><i></i>주문결제</span></li>
                    <li class="active"><span class="step03"><i></i>주문완료</span></li>
                </ul>
            </div>
            <div class="order_completed">
                <h2 class="order_completed_tit"><i></i>주문이 완료되었습니다.</h2>
                <div class="completed_detail">
                    ...
                    <ul class="detail_info">
                        ...
                        <li style="vertical-align:top">
                            <span class="tit" style="vertical-align:top">결제금액</span>
                            <span class="text">
                            <%-- 네이버 스크립트 용 변수&값 --%>
								...
								<c:if test="${paymentList.paymentWayCd ne '01' }">
											...
											<em class="price">
												<c:if test="${paymentList.paymentWayCd eq '23'}"> <%-- 신용카드 --%>
													...
													<span class="bar">|</span>${paymentList.paymentWayNm}<br>
													${paymentList.cardNm}
												</c:if>
												<c:if test="${paymentList.paymentWayCd eq '11'}"> <%-- 무통장 --%>
													...
													<fmt:formatNumber value="${paymentList.paymentAmt}" type="currency" maxFractionDigits="0" currencySymbol=""/>원
													<span class="bar">|</span>${paymentList.paymentWayNm}<br>
													${paymentList.bankNm}&nbsp;${paymentList.actNo}&nbsp;${paymentList.holderNm}<span class="bar">|</span><fmt:formatDate value="${dpstScdDt}" pattern="yyyy-MM-dd" />까지
												</c:if>
												...
												<c:if test="${paymentList.paymentWayCd eq '22'}"> <%-- 가상계좌 --%>
													...
													<fmt:formatNumber value="${paymentList.paymentAmt}" type="currency" maxFractionDigits="0" currencySymbol=""/>원
													<span class="bar">|</span>${paymentList.paymentWayNm}<br>
													${paymentList.bankNm}&nbsp;${paymentList.actNo}<span class="bar">|</span><fmt:formatDate value="${dpstScdDt}" pattern="yyyy-MM-dd" />까지
												</c:if>
                        ...
                        <li>
                            ...
                            <span class="bar">|</span>${orderVO.orderInfoVO.adrsNm}</span>
                        </li>
                    </ul>
                    ...
                    <c:if test="${recomm_show eq 'Y'}">
                        <div class="order_completed_reservation">
                            <p class="text01">
                                <em class="member">${orderVO.orderInfoVO.ordrNm}</em>님, 고객님의 시력과 라이프 스타일에 맞는 ${contact_lens_exist }렌즈를 추천해 드릴게요
                                <!-- 
                                <button type="button" class="btn_recomm_check" onclick="javascript:location.href='/front/vision/vision-check?lensType=${lensType}'">${contact_lens_exist }렌즈 추천받기<i></i></button>
                                 -->
                                <button type="button" class="btn_recomm_check" onclick="javascript:location.href='/front/vision2/vision-check?visionGb=${lensType}'">${contact_lens_exist }렌즈 추천받기<i></i></button>
                            </p>
                ...
                <div class="cart_bottom_btn_area">
                    <button type="button" class="btn_select_checkout" onclick="location.href='/front'">쇼핑계속하기</button>
                    <button type="button" class="btn_all_checkout" id="move_order">주문내역확인</button>
                </div>
            </div>
        </div>
        <!---// 02.LAYOUT: 주문 메인 --->
			...
			<div class="pop_wrap">
			    ...
			    <div class="alert_content">
			        ...
			        <div class="alert_btn_area">
			            ...
			            <!-- 
			            <button type="button" class="btn_alert_ok" onclick="javascript:location.href='/front/vision/vision-check?lensType=${lensType}'">지금 추천받기</button>
			             -->
			            <button type="button" class="btn_alert_ok" onclick="javascript:location.href='/front/vision2/vision-check?visionGb=${lensType}'">지금 추천받기</button>
			        </div>
			    </div>
			</div>
		</div>
