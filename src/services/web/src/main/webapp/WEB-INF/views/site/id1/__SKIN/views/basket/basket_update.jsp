...
$(document).ready(function(){
    ...
    $('.btn_review_ok').on('click', function(){
        ...
        if(formCheck) {
            ...
            param+='&goodsNoArr=${goodsInfo.data.goodsNo}';
...
/* 옵션 재고 확인(옵션O) */
function jsCheckOptionStockQtt_pop(obj) {
    var rtn = true;
...
function jsSetTotalPrice() {
    ...
    var prmtDcValue = "${promotionInfo.data.prmtDcValue}";
    var dcPrice = 0;

    if(prmtDcValue !== null) {
        dcPrice = totalPrice * (prmtDcValue/100)/10;
        dcPrice = (dcPrice - (dcPrice%1))*10;
...
function jsFormValidation() {
    ...
    if(multiOptYn == 'Y') {
        $('[id^=option_layer_]').each(function(){
            stockQttOk = jsCheckOptionStockQtt_pop($(this));
        ...
        stockQttOk = jsCheckOptionStockQtt_pop($('#goods_form'));
    }
    if(!stockQttOk) {
        if(itemNm == '') {
...
function basketInfo(){
    ...
    if(multiOptYn === 'Y') {
        stockQtt = '${itemInfo.stockQtt}';
        itemPrice = '${itemInfo.salePrice}';
    } else {
        stockQtt = '${goodsInfo.data.stockQtt}';
        itemPrice = '${goodsInfo.data.salePrice}';
    }
    ...
    if (attrValue1 != '' && '${itemInfo.optNo1}' !='0') {
        if(itemNm != '') itemNm +=', ';
        itemNm += attrValue1;
    }
    if (attrValue2 != '' && '${itemInfo.optNo2}' !='0') {
        if(itemNm != '') itemNm +=', ';
        itemNm += attrValue2;
    }
    if (attrValue3 != '' && '${itemInfo.optNo3}' !='0') {
        if(itemNm != '') itemNm +=', ';
        itemNm += attrValue3;
    }
    if (attrValue4 != '' && '${itemInfo.optNo4}' !='0') {
        if(itemNm != '') itemNm +=', ';
        itemNm += attrValue4;
    }
...
<!-- layer_popup1 -->
    ...
    <div class="popup_content">
        <!-- 2018-09-04 위치아래로옮김<div class="popup_goods_plus_img">
            <img src="${goodsInfo.data.goodsDispImgC}" alt="">
        </div> -->
        <div class="popup_goods_plus_scroll">
			 <div class="popup_goods_plus_img">
				<img src="${_IMAGE_DOMAIN}${goodsInfo.data.goodsDispImgC}" alt="">
			</div>
        ...
        <c:if test="${goodsStatus eq '01'}">
            ...
            <%-- 단일 옵션일때 input_goods_no가 중복되어 총상품금액 계산이 안되서 빼놓는다  --%>
...
<!--- popup 사은품 보기 --->
<c:if test="${freebieGoodsList ne null && fn:length(freebieGoodsList) gt 0}">
<div class="popup_present_event" id="freebie_pop" style="display: none; top:0; left:0;">
    <div class="popup_header">
