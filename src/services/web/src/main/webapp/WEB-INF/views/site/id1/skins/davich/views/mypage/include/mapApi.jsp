...
//         address.push({"address":"서울특별시 강남구 신사동","storeNo":"0005","storeNm":"신사점"});
 ...
 // 마커를 표시할 위치와 내용을 가지고 있는 객체 배열입니다
    ...
    function getTimeTableList(strDate) {
        ...
        Dmall.AjaxUtil.getJSON(url, param, function(result) {
           ...
           jQuery.each(result.storeChaoticList, function(idx, obj) {
          	 ...
          	 var chaotic = "";
       	 if (obj.chaotic == "01") {
       		 chaotic = '</span><p class="state green"><a href="javascript:void(0);" onclick="setVisitTime(\''+obj.hour+'\',\''+strDate+'\');" >' + obj.chaoticName +'</a></p></li>';
       	 } else if (obj.chaotic == "02") {
       		 chaotic = '</span><p class="state orange"><a href="javascript:void(0);" onclick="setVisitTime(\''+obj.hour+'\',\''+strDate+'\');" >' + obj.chaoticName +'</a></p></li>';
       	 } else {
       		 chaotic = '</span><p class="state red"><a href="javascript:void(0);" onclick="setVisitTime(\''+obj.hour+'\',\''+strDate+'\');" >' + obj.chaoticName +'</a></p></li>';
       	 }
          	 ...
          	 if (Number(idx)%6 == 0) {
          		 ...
          		 str += '<li><span class="time"><a href="javascript:void(0);" onclick="setVisitTime(\''+obj.hour+'\',\''+strDate+'\');" >' + hour + '</a>' + chaotic;
          	 } else {    
          		 str += '<li><span class="time"><a href="javascript:void(0);" onclick="setVisitTime(\''+obj.hour+'\',\''+strDate+'\');" >' + hour + '</a>' + chaotic;
          	 }
   ...
   function setVisitTime(hour, strDate) {
   	 ...
   	 if (strDate == today) {
   		 var timeChk = String(date.getHours() + 1) + date.getMinutes().toString();
   	 }     	 
    	 
   	 if (hour <= timeChk) {
			alert('현재 시간 1시간 이후로 예약시간을 선택할 수 있습니다.');    		 
   		return false;
   	 }
