...
Dmall.FileUpload = {
    ...
    editor : function() {
        return Dmall.FileUpload.fileForm("editor");
    },
    /**
     * <pre>
     * 함수명 : fileForm
    ...
    fileForm : function(type) {
        ...
        switch(type) {
            case  'xls' :
                ...
                break;
            case 'editor' :
                url = '/front/common/editor-image-upload';
                accept = ' accept="image/*"';
                title = '이미지';
                break;
            case 'image' :
                url = '/front/common/image-upload-temp',
                accept = ' accept="image/*"';
                title = '이미지';
                break;
            default :
        ...
        $("#btn_cancel").on('click', function(e) {
            ...
            Dmall.LayerPopupUtil.close('layer_upload_image');
        });
        $("#btn_upload")
            .on('click', function(e) {
                Dmall.EventUtil.stopAnchorAction(e);
    ...
    upload : function(formId) {
        var dfd = jQuery.Deferred(),
            $form = $('#' + formId),
            url = $form.attr("action") || '/front/common/file-upload';
            ...
            if(files && files.length){
                for(var j = 0, l = files.length; j < l; j++) {
                    ...
                    if(file.size > Constant.file.maxSize) {
                        Dmall.LayerUtil.alert('파일 ' + file.name + '의 파일사이즈가 2MB를 초과합니다.');
                        return false;
                    }
                }
            }
...
Dmall.FileDownload = {
     ...
     * 설  명 : 서버의 파일을 다운로드 한다.
     *
     * 사용법 : DmallFileDownload.download('BBS', 1);
     *          DmallFileDownload.download('BBS', '201034040', '1');
     *          DmallFileDownload.download('BBS', '201034040', 'TOP', 123);
     * 작성일 : 2016. 5. 16.
     * 작성자 : dong
     * 수정내역(수정일 수정자 - 수정내용)
     * -------------------------------------
    ...
    download : function(args) {
        var url = '/front/common/common-download',
            key,
            param = {};
